...
with open('append_q.js', 'r', encoding='utf-8') as f:
    code = f.read()

# I will write a simple python script to append JSON objects directly
data = {
  "questions": [
    {
...
q_match = re.search(r'export const gGymnasiouQuestions: Question\[\] = \[([\s\S]*?)\];', content)
mcq_match = re.search(r'export const gGymnasiouMCQ: MCQ\[\] = \[([\s\S]*?)\];', content)

q_content = q_match.group(1).rstrip().rstrip(',')
mcq_content = mcq_match.group(1).rstrip().rstrip(',')

new_q_str = ',\n' + ',\n'.join(['  ' + json.dumps(q, ensure_ascii=False, indent=2).replace('\\n', '\\\\n').replace('\n', '\n  ') for q in data['questions']])
new_mcq_str = ',\n' + ',\n'.join(['  ' + json.dumps(q, ensure_ascii=False, indent=2).replace('\\n', '\\\\n').replace('\n', '\n  ') for q in data['mcqs']])

new_file = f'''import type {{ Question, MCQ }} from "./questions";

export const gGymnasiouQuestions: Question[] = [{q_content}{new_q_str}
];

export const gGymnasiouMCQ: MCQ[] = [{mcq_content}{new_mcq_str}
];
'''
