export interface Question {
  chapterId: string;
  q: string;
  a: string;
}

export interface MCQ {
  chapterId: string;
  q: string;
  options: { text: string; correct: boolean }[];
  explanation: string;
}
export const bGymnasiouQuestions: Question[] = [
  {
    "chapterId": "chapter-1",
    "q": "1. Να συμπληρώσετε τα κενά με τις κατάλληλες λέξεις.\n\nα. Τα υλικά τα οποία ο άνθρωπος χρησιμοποιεί στη μορφή που τα βρίσκει στη φύση ονομάζονται ____________.\n\nβ. Η Χημεία είναι η επιστήμη η οποία μελετά την ____________ της ύλης.\n\nγ. Η Χημεία μελετά τη φύση με σκοπό να εξηγήσει φαινόμενα όπως η ____________ στα φυτά και η ____________ της ύλης στους οργανισμούς.",
    "a": "<span class='text-rose-400 font-bold'>α. φυσικά προϊόντα<br>β. δομή και τις ιδιότητες<br>γ. φωτοσύνθεση, αποικοδόμηση</span>"
  },
  {
    "chapterId": "chapter-1",
    "q": "2. Η φύση είναι μια τεράστια χημική βιομηχανία. Μπορείς να βρεις επιχειρήματα που να στηρίζουν αυτόν τον συλλογισμό;",
    "a": "<span class='text-rose-400 font-bold'>Η φύση λειτουργεί σαν μια πολύπλοκη, αυτορρυθμιζόμενη και αποδοτική χημική βιομηχανία, που παράγει, μετατρέπει και ανακυκλώνει υλικά με τρόπους που συνεχίζουμε να μαθαίνουμε και να εξερευνούμε.<br>Μερικές διαδικασίες που αναδεικνύουν αυτόν το συλλογισμό είναι η φωτοσύνθεση, η ανακύκλωση θρεπτικών στοιχείων με φυσικές διαδικασίες, όπως η αποσύνθεση, ο κύκλος του άζωτου και ο κύκλος του άνθρακα, η χημική επικοινωνία ανάμεσα στους οργανισμούς καθώς και οι διάφορες βιοχημικές αντιδράσεις που υλοποιούνται στους ζωντανούς οργανισμούς.</span>"
  },
  {
    "chapterId": "chapter-1",
    "q": "4. α. Ποιο είναι το αντικείμενο της επιστήμης της Χημείας;\n\nβ. Ποιες μεθόδους χρησιμοποιεί η Χημεία για να επιτύχει τους σκοπούς της;\n\nγ. Να αναφέρετε τέσσερις επιμέρους τομείς στους οποίους έχει οργανωθεί η επιστήμη της Χημείας.\n\nδ. Γιατί λέγεται ότι η Χημεία στηρίζει τη ζωή μας;\n\nΝα αναφέρετε τρεις τομείς της ζωής του ανθρώπου στους οποίους η Χημεία έχει σημαντική προσφορά και να δικαιολογήσετε την απάντησή σας.\n\nε. Υπάρχουν επιζήμιες για τον άνθρωπο συνέπειες από την ανάπτυξη της Χημείας, και αν ναι, να αναφέρετε τουλάχιστον δύο από αυτές.\n\nστ. Να προτείνετε τρόπους για τον περιορισμό των αρνητικών συνεπειών από την ανάπτυξη της Χημείας.",
    "a": "<span class='text-rose-400 font-bold'>α. Η Χημεία είναι η επιστήμη η οποία:<br>i. μελετά τη δομή και τις ιδιότητες της ύλης,<br>ii. ασχολείται με την επεξεργασία πρώτων υλών και τις μεθόδους παραγωγής νέων υλικών,<br>iii. ασχολείται με τον έλεγχο της ποιότητας του περιβάλλοντος και διάφορων προϊόντων, όπως τα τρόφιμα, τα καύσιμα και τα φάρμακα.<br>β. Για να μπορέσει η επιστήμη της Χημείας να καλύψει το αντικείμενό της διεξάγει βασική και εφαρμοσμένη έρευνα. Η βασική έρευνα αφορά στη μελέτη των σωμάτων και των φαινομένων και στη διατύπωση θεωριών που να εξηγούν την συμπεριφορά τους και η εφαρμοσμένη στην εύρεση μεθόδων παραγωγής χρήσιμων προϊόντων και στην ανάπτυξη μεθόδων ελέγχου της ασφάλειας τους.<br>γ. Η επιστήμη της Χημείας έχει οργανωθεί σε επιμέρους τομείς, όπως η Ανόργανη Χημεία, η Οργανική Χημεία, η Αναλυτική Χημεία, η Βιοχημεία, η Φυσικοχημεία, η Χημεία Τροφίμων, η Χημεία Περιβάλλοντος, η Φαρμακοχημεία, η Βιομηχανική Χημεία κ.α.<br>δ. Η επιστήμη της Χημείας μελετώντας την φύση και την δομή των υλικών σωμάτων συνετέλεσε στο να εξηγηθεί το φαινόμενο της ζωής. Ορισμένοι τομείς της ζωής του ανθρώπου στους οποίους η Χημεία έχει σημαντική προσφορά είναι:<br>i. Υγεία και Φαρμακευτική<br>ii. Τρόφιμα και Γεωργία<br>iii. Ενέργεια<br>ε. Η επιστήμη της Χημείας πέτυχε την παραγωγή πολλών νέων προϊόντων τα οποία όμως δεν χρησιμοποιούνται πάντοτε με τρόπο επωφελή για τον άνθρωπο και το περιβάλλον. Δύο αρνητικές συνέπειες της χρήσης των χημικών προϊόντων είναι τα χημικά προϊόντα για πολεμικούς σκοπούς και τα ναρκωτικά και οι εξαρτησιογόνες ουσίες.<br>στ. Με τη συνδυασμένη εφαρμογή αυτών των μέτρων, μπορεί να επιτευχθεί μια πιο βιώσιμη και υγιής ανάπτυξη της χημείας που θα μειώσει τις αρνητικές επιπτώσεις στην κοινωνία και το περιβάλλον. Ορισμένοι τρόποι για τον περιορισμό των αρνητικών συνεπειών από την ανάπτυξη της χημείας περιλαμβάνουν:<br>i. Πρακτικές που προωθούν την αειφορία με σχεδίαση χημικών διεργασιών και προϊόντων που ελαχιστοποιούν τη χρήση των πόρων, την παραγωγή αποβλήτων και τις επιβλαβείς επιπτώσεις στο περιβάλλον και την υγεία.<br>ii. Εκπαίδευση και ευαισθητοποίηση των πολιτών για τις επιπτώσεις της χημείας στο περιβάλλον.<br>iii. Θέσπιση κανονισμών και νόμων με τους οποίους θα ελέγχεται η χρήση ορισμένων χημικών ουσιών, με στόχο την προστασία του περιβάλλοντος.<br>iv. Επένδυση στην εκπαίδευση καθώς και σε ερευνητικά προγράμματα τα οποία θα επιδιώκουν να βελτιώσουν την ασφάλεια των χημικών διεργασιών και των προϊόντων.</span>"
  },
  {
    "chapterId": "chapter-1",
    "q": "5. α. Οι δύο όψεις του ίδιου νομίσματος: Για τα προϊόντα της πρώτης στήλης του ακόλουθου πίνακα να σκεφτείτε και να αντιπαραθέσετε χρήσεις που είναι θετικές για την κοινωνία και χρήσεις που έχουν αρνητικές συνέπειες.\n\nβ. Πού οφείλονται, κατά την εκτίμησή σας, οι επιζήμιες συνέπειες της χρήσης των χημικών ουσιών και πώς θα μπορούσε να διορθωθεί το πρόβλημα;",
    "a": "<span class='text-rose-400 font-bold'>Α. Προϊόντα &amp; Συνέπειες:<br>- Λιπάσματα: Θετικές - Αύξηση παραγωγής τροφίμων. Αρνητικές - Ρύπανση θάλασσας, λιμνών, ποταμών και υπόγειων υδάτων.<br>- Πλαστικά: Θετικές - Είναι φθηνά, ανθεκτικά και ελαφριά. Αρνητικές - Η αλόγιστη χρήση τους επιβαρύνει το περιβάλλον λόγω της μεγάλης τους αντοχής.<br>- Εντομοκτόνα: Θετικές - Αντιμετώπιση ασθενειών των φυτών και αύξηση της γεωργικής παραγωγής. Αρνητικές - Αρνητικές επιπτώσεις στην υγεία των καταναλωτών. Χρησιμοποιήθηκαν και για πολεμικούς σκοπούς.<br>- Χλώριο: Θετικές - Ισχυρό απολυμαντικό. Αρνητικές - Πολεμικό αέριο.<br>- Συντηρητικά τροφίμων: Θετικές - Παράταση χρόνου ζωής των τροφίμων ώστε να αποθηκεύονται για μεγαλύτερο χρόνο. Αρνητικές - Ορισμένα σε μεγάλες ποσότητες μπορεί να συνδεθούν με υγειονομικά προβλήματα.<br>- Φάρμακα: Θετικές - Βελτιώνουν την υγεία και τη ζωή των ανθρώπων, αυξάνουν τον προσδόκιμο χρόνο ζωής. Αρνητικές - Η άκριτη χρήση τους οδηγεί σε εξάρτηση και ακύρωση της ευεργετικής τους δράσης.<br>- Εκρηκτικά: Θετικές - Χρήση σε βιομηχανία και κατασκευές. Αρνητικές - Χρησιμοποιούνται για πολεμικούς σκοπούς.<br><br>Β. Για την αντιμετώπιση των επιζήμιων συνεπειών της χρήσης των χημικών ουσιών, μπορούν να ληφθούν μια σειρά μέτρων όπως:<br>- Η ανάπτυξη και η χρήση χημικών ουσιών που είναι λιγότερο επιβλαβείς για την υγεία και το περιβάλλον.<br>- Ενίσχυση των ελέγχων με αυστηρότερες προδιαγραφές ώστε να διασφαλιστεί η ασφάλεια και η προστασία της υγείας και του περιβάλλοντος.<br>- Εκπαίδευση και ευαισθητοποίηση αγροτών, εργαζομένων και πολιτών σχετικά με τους κινδύνους της χρήσης χημικών ουσιών.<br>- Ενίσχυση της έρευνας για νέα υλικά.</span>"
  },
  {
    "chapterId": "chapter-1",
    "q": "6. Να συμπληρώσετε τα κενά με τις κατάλληλες λέξεις.\n\nΗ ανακάλυψη υλικών, όπως τα ____________ και τα ____________, βελτίωσε σημαντικά την υγεία των ανθρώπων και είχε ως αποτέλεσμα την αύξηση του ____________ της ζωής.\n\nΕπίσης, η χρήση των ____________ αύξησε σημαντικά τη γεωργική παραγωγή και η χρήση των ____________ και των ____________ βελτίωσε τις συνθήκες υγιεινής.",
    "a": "<span class='text-rose-400 font-bold'>φάρμακα, απολυμαντικά, προσδόκιμου, λιπασμάτων, σαπουνιών, απορρυπαντικών</span>"
  },
  {
    "chapterId": "chapter-1",
    "q": "7. Να αντιστοιχίσετε τα υλικά της πρώτης στήλης με το ή τα προϊόντα και τις δραστηριότητες στις οποίες αξιοποιείται στη δεύτερη στήλη.\n\nΥΛΙΚΟ:\n1. χάλυβας (κράμα σιδήρου)\n2. τεχνητό μετάξι (ρεγιόν)\n3. βενζίνη\n4. ασπιρίνη\n5. λίπασμα\n6. πλαστικό\n7. χρώμα\n8. χαρτί\n\nΠροϊόν - Δραστηριότητα:\nΒ<sub>1</sub>. μεταφορές - ταξίδια\nΒ<sub>2</sub>. καλλιέργειες\nΒ<sub>3</sub>. υλικά συσκευασίας\nΒ<sub>4</sub>. ζωγραφική\nΒ<sub>5</sub>. κατασκευές\nΒ<sub>6</sub>. ανακούφιση πυρετού\nΒ<sub>7</sub>. ρούχα\nΒ<sub>8</sub>. μπάλες ποδοσφαίρου",
    "a": "\n<div class='overflow-x-auto my-4'>\n  <table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n    <thead class='bg-white/10'>\n      <tr>\n        <th class='py-3 px-4 font-semibold'>ΥΛΙΚΟ</th>\n        <th class='py-3 px-4 font-semibold'>Προϊόν - Δραστηριότητα</th>\n        <th class='py-3 px-4 font-semibold text-emerald-400'>Απαντήσεις</th>\n      </tr>\n    </thead>\n    <tbody class='divide-y divide-white/10'>\n      <tr class='hover:bg-white/5 transition-colors'>\n        <td class='py-3 px-4'>1. χάλυβας (κράμα σιδήρου)</td>\n        <td class='py-3 px-4'>Β1. μεταφορές - ταξίδια</td>\n        <td class='py-3 px-4 text-emerald-400 font-bold'>1 → Β5</td>\n      </tr>\n      <tr class='hover:bg-white/5 transition-colors'>\n        <td class='py-3 px-4'>2. τεχνητό μετάξι (ρεγιόν)</td>\n        <td class='py-3 px-4'>Β2. καλλιέργειες</td>\n        <td class='py-3 px-4 text-emerald-400 font-bold'>2 → Β7</td>\n      </tr>\n      <tr class='hover:bg-white/5 transition-colors'>\n        <td class='py-3 px-4'>3. βενζίνη</td>\n        <td class='py-3 px-4'>Β3. υλικά συσκευασίας</td>\n        <td class='py-3 px-4 text-emerald-400 font-bold'>3 → Β1</td>\n      </tr>\n      <tr class='hover:bg-white/5 transition-colors'>\n        <td class='py-3 px-4'>4. ασπιρίνη</td>\n        <td class='py-3 px-4'>Β4. ζωγραφική</td>\n        <td class='py-3 px-4 text-emerald-400 font-bold'>4 → Β6</td>\n      </tr>\n      <tr class='hover:bg-white/5 transition-colors'>\n        <td class='py-3 px-4'>5. λίπασμα</td>\n        <td class='py-3 px-4'>Β5. κατασκευές</td>\n        <td class='py-3 px-4 text-emerald-400 font-bold'>5 → Β2</td>\n      </tr>\n      <tr class='hover:bg-white/5 transition-colors'>\n        <td class='py-3 px-4'>6. πλαστικό</td>\n        <td class='py-3 px-4'>Β6. ανακούφιση πυρετού</td>\n        <td class='py-3 px-4 text-emerald-400 font-bold'>6 → Β1, Β3, Β8</td>\n      </tr>\n      <tr class='hover:bg-white/5 transition-colors'>\n        <td class='py-3 px-4'>7. χρώμα</td>\n        <td class='py-3 px-4'>Β7. ρούχα</td>\n        <td class='py-3 px-4 text-emerald-400 font-bold'>7 → Β3, Β4, Β5, Β7</td>\n      </tr>\n      <tr class='hover:bg-white/5 transition-colors'>\n        <td class='py-3 px-4'>8. χαρτί</td>\n        <td class='py-3 px-4'>Β8. μπάλες ποδοσφαίρου</td>\n        <td class='py-3 px-4 text-emerald-400 font-bold'>8 → Β3, Β4</td>\n      </tr>\n    </tbody>\n  </table>\n</div>\n"
  },
  {
    "chapterId": "chapter-3",
    "q": "1. Το μεγαλύτερο ποσοστό του νερού στη Γη είναι ____________ και αποτελεί περίπου το 97% της συνολικής ποσότητας του νερού.\n\nΤο υπόλοιπο 3% είναι ____________ νερό, αλλά δεν μπορεί να χρησιμοποιηθεί όλο γιατί το μεγαλύτερο ποσοστό του βρίσκεται στους παγετώνες.\n\nΈτσι περίπου το 1% του συνολικού νερού μπορεί αξιοποιηθεί για τις ανάγκες του ανθρώπου.\n\nΤο νερό που προορίζεται για ανθρώπινη κατανάλωση ονομάζεται ____________ νερό και πρέπει να είναι καθαρό, δηλαδή να μην έχει μεγάλη ____________, δηλαδή πολλά άλατα, ούτε μεγάλη ποσότητα οργανικών ουσιών, βαρέων μετάλλων και μικροβίων, ώστε να μπορεί να καταναλώνεται χωρίς να κινδυνεύει η υγεία του ανθρώπου.",
    "a": "<span class='text-rose-400 font-bold'>αλμυρό, γλυκό, πόσιμο, σκληρότητα</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "3. Να δικαιολογήσετε τη φράση: «Στα αφυδατωμένα τρόφιμα δεν επιβιώνουν οι μικροοργανισμοί και γι' αυτό διατηρούνται για μεγαλύτερο διάστημα».",
    "a": "<span class='text-rose-400 font-bold'>Οι μικροοργανισμοί, όπως όλοι οι οργανισμοί, απαιτούν νερό για την επιβίωση και την ανάπτυξή τους. Λόγω της μειωμένης ποσότητας νερού που διαθέτουν τα αφυδατωμένα τρόφιμα, οι συνθήκες για την ανάπτυξη μικροοργανισμών που καταστρέφουν τα τρόφιμα ελαττώνονται σημαντικά.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "4. Ζυγίζουμε μια ποσότητα φρέσκα βερίκοκα και τη βρίσκουμε ίση με 100 g. Τοποθετούμε τα βερίκοκα στον ήλιο για αρκετές μέρες και παρατηρούμε ότι συρρικνώνονται (ζαρώνουν) και όταν τα ζυγίζουμε βρίσκουμε τη μάζα τους 60 g. Πού οφείλεται η μεταβολή στην εμφάνιση και τη μάζα των βερίκοκων;",
    "a": "<span class='text-rose-400 font-bold'>Η μεταβολή στην εμφάνιση και τη μάζα των βερίκοκων οφείλεται στην εξάτμιση μέρους του νερού που περιέχουν (100g - 60g = 40g νερό εξατμίζεται).</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "5. Σε γλάστρες που περιέχουν φυτά πολύ μικρής απαίτησης σε Η₂Ο, όπως οι κάκτοι, τοποθετούνται μερικές φορές στην επιφάνεια του χώματος κρύσταλλοι γαλαζόπετρας. Η σύσταση των γεωπόνων είναι να παρακολουθείται το χρώμα των κρυστάλλων και όταν αρχίσουν να χάνουν το γαλάζιο χρώμα τους και να γίνονται λευκοί, το φυτό να ποτίζεται.\n\nα. Τι έχει συμβεί όταν η γαλαζόπετρα χάνει το χρώμα της;\n\nβ. Πώς θα μπορούσαμε να ελέγξουμε αν η ατμόσφαιρα περιέχει υδρατμούς;",
    "a": "<span class='text-rose-400 font-bold'>Α. Έχει εξατμιστεί το νερό για αυτό και ο ένυδρος θειικός χαλκός που έχει γαλάζιο χρώμα έχει μετατραπεί σε άνυδρο θειικό χαλκό που έχει λευκό χρώμα.<br>Β. Για την ανίχνευση των υδρατμών της ατμόσφαιρας μπορεί να χρησιμοποιηθεί ένα παγωμένο ποτήρι ή μπουκάλι ή μεταλλικό κουτί. Επάνω σε αυτό θα σχηματιστούν σταγόνες νερού, από την υγροποίηση των υδρατμών.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "6. Το χλωρίδιο του κοβαλτίου (CoCl<sub>2</sub>) είναι ένα άλας το οποίο όταν είναι άνυδρο έχει μπλε χρώμα και όταν είναι ένυδρο είναι ροζ. Να προτείνετε ένα πείραμα με το οποίο να αποδεικνύεται ότι το σώμα μας αποβάλλει νερό.",
    "a": "<span class='text-rose-400 font-bold'>Μπορούμε να φυσήξουμε (εκπνεύσουμε) πάνω σε άνυδρο χλωρίδιο του κοβαλτίου. Οι υδρατμοί της εκπνοής μας θα απορροφηθούν από το άλας, και αυτό θα μετατραπεί από μπλε σε ροζ, αποδεικνύοντας ότι με την αναπνοή μας αποβάλλουμε νερό.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "<div class='flex flex-col md:flex-row gap-4'><div class='flex-1'>7. Να παρατηρήσετε προσεκτικά τη διπλανή εικόνα (Ανίχνευση νερού σε τρόφιμα) και να προσπαθήσετε να εξηγήσετε τι δείχνει και γιατί.</div><div class='w-full md:w-1/3'><div class='bg-white/10 p-4 rounded-xl text-center border border-dashed border-white/30 text-white/60'>[Προσθέστε την εικόνα water.png εδώ]</div></div></div>",
    "a": "<span class='text-rose-400 font-bold'>Η εικόνα δείχνει ότι όταν θερμαίνουμε ένα τρόφιμο σε δοχείο, το νερό που περιέχει εξατμίζεται. Αν φέρουμε σε επαφή τους υδρατμούς με λευκό άνυδρο θειικό χαλκό, αυτός θα απορροφήσει το νερό και θα γίνει ένυδρος θειικός χαλκός, αποκτώντας το χαρακτηριστικό γαλάζιο χρώμα (γαλαζόπετρα). Έτσι αποδεικνύεται η ύπαρξη νερού στο τρόφιμο.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "7. Να επιλέξετε ποιες από τις ακόλουθες προτάσεις είναι σωστές και ποιες λανθασμένες, και να αιτιολογήσετε την απάντησή σας.\n\nα. Μπορούμε να ανιχνεύσουμε την παρουσία του νερού στον ατμοσφαιρικό αέρα χρησιμοποιώντας γαλαζόπετρα, δηλαδή μπλε ένυδρο θειικό χαλκό.\n\nβ. Το γάλα δεν περιέχει νερό.\n\nγ. Το λάδι δεν περιέχει νερό.\n\nδ. Αν μια βενζίνη περιέχει νερό είναι νοθευμένη.",
    "a": "<span class='text-rose-400 font-bold'>α. Η πρόταση είναι σωστή. Ο άνυδρος θειικός χαλκός (CuSO<sub>4</sub>) είναι ένα άλας σε μορφή λευκής σκόνης. Όταν όμως απορροφήσει νερό μετατρέπεται σε ένυδρο θειικό χαλκό (CuSO<sub>4</sub>·5H<sub>2</sub>O) το οποίο έχει ένα χαρακτηριστικό γαλάζιο χρώμα. Η αλλαγή στο χρώμα μάς επιτρέπει να το χρησιμοποιήσουμε για την ανίχνευση του νερού στον ατμοσφαιρικό αέρα.<br>β. Η πρόταση είναι λανθασμένη. Το νερό είναι το κύριο συστατικό του γάλακτος. Αυτό μπορεί να πιστοποιηθεί αν σε δείγμα γάλακτος προσθέσουμε λευκό άνυδρο θειικό χαλκό όπου θα μετατραπεί σε γαλάζιο ένυδρο θειικό χαλκό.<br>γ. Η πρόταση είναι σωστή.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "4. α. Πώς κατέληξε ο Lavoisier στο συμπέρασμα ότι το νερό είναι μία χημική ένωση;\n\nβ. Ποιες πειραματικές μεθόδους μπορούμε να χρησιμοποιήσουμε για να διαπιστώσουμε ποια είναι τα στοιχεία από τα οποία αποτελείται το νερό;\n\nγ. Αν κατά τη διάσπαση ορισμένης ποσότητας νερού παράγονται 2 L ενός αερίου Α και 4 L ενός αερίου Β, ποιο αέριο είναι το Α και ποιο το Β, και γιατί;\n\nδ. Αν κατά τη διάσπαση ορισμένης ποσότητας νερού παράγονται 4 g οξυγόνου, πόσα g υδρογόνου παράγονται ταυτόχρονα και πόσα g νερού διασπάστηκαν; Να δικαιολογήσετε την απάντησή σας.",
    "a": "<span class='text-rose-400 font-bold'>α. Ο Lavoisier κατέληξε στο συμπέρασμα ότι το νερό είναι μία χημική ένωση με ένα πείραμα με το οποίο παρασκεύασε νερό από υδρογόνο και οξυγόνο, δηλαδή από απλούστερες χημικές ουσίες.<br>β. Τα στοιχεία από τα οποία αποτελείται το νερό είναι το οξυγόνο και το υδρογόνο. Για να ανιχνεύσουμε την ύπαρξη του οξυγόνου παίρνουμε ένα μακρύ και λεπτό ξυλάκι (παρασχίδα) και, αφού το ανάψουμε και παραχθεί φλόγα, το σβήνουμε, αφήνοντας ωστόσο το άκρο του πυρωμένο (καύτρα). Στη συνέχεια εισάγουμε τη μισοσβησμένη παρασχίδα μέσα στον δοκιμαστικό σωλήνα όπου παράγεται οξυγόνο. Παρατηρούμε ότι η παρασχίδα αναφλέγεται, δηλαδή ενισχύεται η καύση της.<br>Για να ανιχνεύσουμε την ύπαρξη του υδρογόνου σε κωνική φιάλη που συνδέεται με ποτήρι ζέσεως μέσα στο οποίο έχουμε προσθέσει σαπουνόνερο, βάζουμε μερικά κομματάκια ψευδάργυρο και υδροχλωρικό οξύ και παρατηρούμε να συγκεντρώνονται φυσαλίδες αερίου πάνω από το σαπουνόνερο. Απομακρύνουμε την κωνική φιάλη και πλησιάζουμε στις φυσαλίδες μια αναμμένη παρασχίδα. Θα ακούσουμε έναν χαρακτηριστικό κρότο, σαν μικρή έκρηξη, και το υδρογόνο θα καεί με έντονη φλόγα.<br>γ. Κατά την διάσπαση του νερού ο όγκος του υδρογόνου που παράγεται είναι διπλάσιος από τον όγκο του οξυγόνου. Επομένως το αέριο Α είναι το οξυγόνο και το αέριο Β είναι το υδρογόνο.<br>Δ. Κατά τη διάσπαση του νερού η μάζα του οξυγόνου που παράγεται είναι οκταπλάσια της μάζας του υδρογόνου. Επομένως αν παράγονται 4 g οξυγόνου, τότε θα παράγονται και 0,5 g υδρογόνου και θα διασπάστηκαν 4,5 g νερού.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "8. Ο Ίον έχει μάζα 75 kg και στην προηγούμενη ενότητα μάθαμε ότι το 66% της μάζας του ανθρώπινου σώματος είναι νερό. Επομένως, στο σώμα του το νερό έχει μάζα 49,5 kg.\n\nΜε δεδομένο ότι η αναλογία μαζών υδρογόνου - οξυγόνου στο νερό είναι σταθερή και ίση με ____________, η μάζα του υδρογόνου στο νερό στο σώμα του Ίονα είναι ____________ kg και η μάζα του οξυγόνου στο νερό στο σώμα του Ίονα είναι ____________ kg.",
    "a": "<span class='text-rose-400 font-bold'>1/8, 5,5, 44</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "<div class='flex flex-col md:flex-row gap-4 mb-4'><div class='flex-1'>Παρατηρήστε τη διπλανή εικόνα και τους αντίστοιχους ισχυρισμούς.<br>Έχει δίκιο ο Σώτος ότι δεν θα κουραστεί ο Μέντιος;<br>Ένα διάλυμα ζυγίζει λιγότερο από όσο ζυγίζουν τα υλικά του πριν από την ανάμειξη;<br><br>α. Ναι [ ] &nbsp;&nbsp; β. Όχι [ ] &nbsp;&nbsp; γ. Εξαρτάται [ ]<br><br>Να σχεδιάσετε ένα πείραμα με το οποίο θα αποδείξετε την απάντησή σας.<br>Έχετε στη διάθεσή σας: ηλεκτρονική ζυγαριά, ποτήρι ζέσεως, ζάχαρη, ύαλο ωρολογίου, νερό, σπάτουλα.<br><br>Ο σάκος του υπεραθλητή Cani Recori θα γίνει πιο ελαφρύς αν διαλύσει τη σκόνη ηλεκτρολυτών στο μπουκάλι με το νερό του, αντί να την έχει δίπλα σε αυτό;</div><div class='w-full md:w-1/3'><div class='bg-white/10 p-4 rounded-xl text-center border border-dashed border-white/30 text-white/60'>[Προσθέστε την εικόνα sotos.png εδώ]</div></div></div>",
    "a": "<span class='text-rose-400 font-bold'>Όχι, δεν έχει δίκιο ο Σώτος. Η μάζα ενός διαλύματος ισούται με το άθροισμα των μαζών του διαλύτη και της διαλυμένης ουσίας (m<sub>διαλύματος</sub> = m<sub>διαλύτη</sub> + m<sub>διαλ. ουσίας</sub>), λόγω της αρχής διατήρησης της μάζας. Επομένως, το συνολικό βάρος που κουβαλάει ο Μέντιος δεν θα αλλάξει.<br><br><b>Πείραμα:</b> Ζυγίζουμε το ποτήρι ζέσεως με το νερό και τη ζάχαρη στην ύαλο ωρολογίου χωριστά, και βρίσκουμε το άθροισμα των μαζών τους. Στη συνέχεια, ρίχνουμε τη ζάχαρη στο νερό, ανακατεύουμε με τη σπάτουλα μέχρι να διαλυθεί πλήρως, και ζυγίζουμε το ποτήρι ζέσεως με το διάλυμα. Θα διαπιστώσουμε ότι η μάζα του διαλύματος είναι ίση με το άθροισμα των αρχικών μαζών.<br><br>Για τον αθλητή: Όχι, το συνολικό βάρος θα παραμείνει ίδιο.</span>"
  },

  {
    "chapterId": "chapter-3",
    "q": "Να συμπληρώσετε τα κενά με τις κατάλληλες λέξεις: α. Η βενζίνη είναι ένα ... μείγμα, διότι αποτελείται από πολλούς υδρογονάνθρακες οι οποίοι δεν διακρίνονται μεταξύ τους. β. Αν αναμείξουμε λάδι με νερό, θα παρατηρήσουμε ότι θα σχηματιστούν δύο διακριτά στρώματα στο ποτήρι. Το σύστημα λάδι-νερό είναι ένα ... μείγμα.",
    "a": "<span class='text-purple-300 font-bold'>α. ομογενές<br>β. ετερογενές</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Να συμπληρώσετε τα κενά με τις κατάλληλες λέξεις: α. Το οινόπνευμα του εμπορίου περιέχει νερό και αιθανόλη. Παρότι η αιθανόλη βρίσκεται σε μεγαλύτερη αναλογία, το οινόπνευμα χαρακτηρίζεται ... β. Η ποσότητα μιας ουσίας που μπορεί να διαλυθεί στο νερό που περιέχεται σε ένα ποτήρι εξαρτάται από: 1. ... 2. ...",
    "a": "<span class='text-purple-300 font-bold'>α. υδατικό διάλυμα<br>β. 1. φύση της ουσίας, 2. θερμοκρασία</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Ο Ίον πειραματίζεται στο σπίτι του φτιάχνοντας διαλύματα ζαχαρόνερου. Διέλυσε αρχικά ένα κουταλάκι ζάχαρη, μετά δεύτερο, αλλά το τρίτο δεν διαλύθηκε στο κρύο νερό. Με ζεστό νερό διαλύθηκαν και τα τρία. Να δικαιολογήσετε γιατί: α. Το διάλυμα με το πρώτο κουταλάκι είχε γλυκιά γεύση. β. Το διάλυμα με το δεύτερο κουταλάκι είχε πιο γλυκιά γεύση. γ. Δεν είναι δυνατόν να διαλυθούν τρία κουταλάκια σε ένα ποτήρι κρύο νερό. δ. Πώς επηρεάζει η θερμοκρασία; ε. Πώς θα μπορούσε να διαλυθεί η επιπλέον ζάχαρη; στ. Ποιο διάλυμα είναι το πιο πυκνό;",
    "a": "<span class='text-purple-300 font-bold'>\n    <ul class='list-disc ml-5 mt-2'>\n      <li class='mb-2'>α. Τα συστατικά ενός μείγματος συνήθως διατηρούν τις ιδιότητές τους, οπότε το διάλυμα είναι γλυκό.</li>\n      <li class='mb-2'>β. Το διάλυμα έχει περισσότερη ζάχαρη οπότε η γεύση είναι πιο γλυκιά.</li>\n      <li class='mb-2'>γ. Η ποσότητα μιας ουσίας που μπορεί να διαλυθεί σε ορισμένη ποσότητα ενός διαλύτη δεν είναι απεριόριστη. Η μέγιστη αυτή ποσότητα ονομάζεται διαλυτότητα.</li>\n      <li class='mb-2'>δ. Με αύξηση της θερμοκρασίας αυξάνεται η ποσότητα της ζάχαρης που μπορεί να διαλυθεί.</li>\n      <li class='mb-2'>ε. Μεγαλύτερη ποσότητα νερού μπορεί να διαλύσει μεγαλύτερη ποσότητα ζάχαρης.</li>\n      <li class='mb-2'>στ. Ένα διάλυμα που περιέχει μεγάλη ποσότητα διαλυμένης ουσίας χαρακτηρίζεται πυκνό, οπότε το πιο πυκνό είναι αυτό με τα τρία κουταλάκια.</li>\n    </ul></span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Να συμπληρώσετε τα κενά με τις κατάλληλες λέξεις: i. Το οινόπνευμα του εμπορίου περιέχει νερό και αιθανόλη. Η αιθανόλη βράζει στους 78°C και το νερό στους 100°C. Η κατάλληλη μέθοδος διαχωρισμού τους είναι η ... ii. Η κατάλληλη μέθοδος για τον διαχωρισμό α. καρπών φακής από πετραδάκια είναι η ... β. μαγειρικού αλατιού από άμμο είναι αρχικά η ... και στη συνέχεια η ...",
    "a": "<span class='text-purple-300 font-bold'>i. απόσταξη<br>ii. α. διαλογή<br>β. διάλυση με νερό (γιατί μόνο το αλάτι διαλύεται) και στη συνέχεια η διήθηση του διαλύματος, ώστε να παραμείνει ως ίζημα η άμμος.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "α. Ποια μέθοδο διαχωρισμού πιστεύετε ότι χρησιμοποιεί η βιομηχανία γάλακτος για να αποβουτυρώνει το γάλα και να παράγει γάλα χαμηλών λιπαρών, και πώς το επιτυγχάνει; β. Να περιγράψετε τη διαδικασία με την οποία μπορείτε να βρείτε ποιες χρωστικές έχουν χρησιμοποιηθεί για το μελάνι ενός ροζ ή τιρκουάζ μαρκαδόρου.",
    "a": "<span class='text-purple-300 font-bold'>\n    <ul class='list-disc ml-5 mt-2'>\n      <li class='mb-2'>α. Η μέθοδος είναι η φυγοκέντριση. Το γάλα διαχωρίζεται στα διαφορετικά συστατικά του (όπως οι πρωτεΐνες, το λίπος και νερό) με βάση τη διαφορετική τους πυκνότητα.</li>\n      <li class='mb-2'>β. Μπορούμε να βρούμε τις χρωστικές με τη μέθοδο της χρωματογραφίας, όπου τα συστατικά του μείγματος κινούνται με διαφορετική ταχύτητα σε έναν διαλύτη όπως το διηθητικό χαρτί.</li>\n    </ul></span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Με βάση την εικόνα μιας πειραματικής διάταξης με χωνί και ηθμό: α. Ποια μέθοδο διαχωρισμού μειγμάτων αναπαριστά το σχήμα και τι είδους μείγματα διαχωρίζονται; β. Να περιγράψετε σύντομα τη διαδικασία. γ. Ποια μείγματα (Αλατόνερο, Ρόφημα χαμομήλι με τα άνθη του, Πλήρες γάλα, Νερό με χώμα, Ζαχαρόνερο) μπορούν να διαχωριστούν;",
    "a": "<span class='text-purple-300 font-bold'>\n    <ul class='list-disc ml-5 mt-2'>\n      <li class='mb-2'>α. Η μέθοδος είναι η διήθηση. Με την μέθοδο αυτή διαχωρίζουμε ετερογενή μείγματα στερεού - υγρού.</li>\n      <li class='mb-2'>β. Κατασκευάζουμε έναν πτυχωτό ηθμό και τον τοποθετούμε στο χωνί. Εφαρμόζοντας τη γυάλινη ράβδο στο ρύγχος του ποτηριού ως οδηγό, αδειάζουμε το μείγμα στον ηθμό. Στη συνέχεια συλλέγουμε το διήθημα σε ποτήρι ζέσεως ενώ στον ηθμό μένει το αδιάλυτο στερεό.</li>\n    </ul>\n    Τα μείγματα που μπορούν να διαχωριστούν με διήθηση (σύμφωνα με τον πίνακα) είναι:\n    <div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n      <thead class='bg-white/10'>\n        <tr class='border-b border-white/20'>\n          <th class='py-3 px-4 font-semibold'>Μείγμα</th>\n          <th class='py-3 px-4 font-semibold'>Μπορεί να διαχωριστεί</th>\n        </tr>\n      </thead>\n      <tbody class=\"divide-y divide-white/10\">\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Αλατόνερο</td><td class='py-3 px-4 text-rose-400'>Όχι</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Ρόφημα χαμομήλι με τα άνθη του</td><td class='py-3 px-4 text-emerald-400'>Ναι (+)</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Πλήρες γάλα</td><td class='py-3 px-4 text-rose-400'>Όχι</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Νερό με χώμα</td><td class='py-3 px-4 text-emerald-400'>Ναι (+)</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Ζαχαρόνερο</td><td class='py-3 px-4 text-rose-400'>Όχι</td></tr>\n      </tbody>\n    </table></div></span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "17. Να συμπληρώσετε την ακροστιχίδα. Η λέξη στην ακροστιχίδα δηλώνει τον στόχο που έχουμε όταν εκχυλίζουμε, αποστάζουμε, διηθούμε ένα μείγμα (στην αιτιατική).\n1. Μέθοδος διαχωρισμού στερεού από υγρό σε ετερογενές μείγμα.\n2. Μία τέτοια αξιοποιούμε για να διαχωρίσουμε ένα μείγμα στα συστατικά του.\n3. Μέθοδος διαχωρισμού στερεού - υγρού, όταν θέλουμε να παραλάβουμε και τα δύο.\n4. Η μέθοδος που χρησιμοποιείται για τον διαχωρισμό των χρωστικών της μελάνης.\n5. Ένα από τα φωνήεντα των χρωστικών.\n6. Αυτό του τσαγιού παραλαμβάνεται με εκχύλιση των αιθέριων ελαίων των φύλλων του φυτού.\n7. Το στερεό υπόλειμμα.\n8. Τέτοια πρέπει να είναι η διαλυμένη ουσία για να διαχωριστεί με εξάτμιση του διαλύτη.\n9. Το αποτέλεσμα της ανάμειξης ουσιών.\n10. Τέτοιο μείγμα είναι το διάλυμα του αλατόνερου.",
    "a": "<div class='overflow-x-auto my-4'><table class='border-collapse text-sm'><thead><tr><th class='py-2 px-3 text-left text-white/60 font-normal'>#</th><th class='py-2 px-3 text-left text-white/60 font-normal'>Απάντηση</th></tr></thead><tbody><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>1.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Δ</span><span class='text-emerald-300'>ΙΗΘΗΣΗ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>2.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ι</span><span class='text-emerald-300'>ΔΙΟΤΗΤΑ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>3.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Α</span><span class='text-emerald-300'>ΠΟΣΤΑΞΗ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>4.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Χ</span><span class='text-emerald-300'>ΡΩΜΑΤΟΓΡΑΦΙΑ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>5.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ω</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>6.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ρ</span><span class='text-emerald-300'>ΟΦΗΜΑ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>7.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ι</span><span class='text-emerald-300'>ΖΗΜΑ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>8.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Σ</span><span class='text-emerald-300'>ΤΕΡΕΗ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>9.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Μ</span><span class='text-emerald-300'>ΕΙΓΜΑ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>10.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ο</span><span class='text-emerald-300'>ΜΟΓΕΝΕΣ</span></td></tr></tbody></table></div><p class='mt-3 text-yellow-300 font-bold text-base'>Η λέξη της ακροστιχίδας είναι: <span class='tracking-widest text-yellow-400 text-lg'>ΔΙΑΧΩΡΙΣΜΟ</span></p>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Τι πληροφορίες μάς δίνει η έκφραση: Η περιεκτικότητα της μαγιονέζας σε λιπαρά είναι 80% μάζα προς μάζα;",
    "a": "<span class='text-purple-300 font-bold'>Σε 100 g μαγιονέζας περιέχονται 80 g λιπαρών.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Κάποιο αλλαντικό αναγράφει στη συσκευασία του: Σε 100 g έτοιμου προϊόντος περιέχονται: πρωτεΐνες 10,6 g, λιπαρά 35 g, αλάτι 0,8 g. α. Ποια είναι η % μάζα προς μάζα περιεκτικότητα του αλλαντικού σε αυτές τις ουσίες; β. Τι πληροφορίες μάς δίνει η ετικέτα ενός γιαουρτιού light η οποία αναφέρει 2% μάζα προς μάζα λιπαρά;",
    "a": "<span class='text-purple-300 font-bold'>α. Το αλλαντικό έχει περιεκτικότητα 10,6% μάζα προς μάζα σε πρωτεΐνες, 35% μάζα προς μάζα σε λιπαρά, 0,8% μάζα προς μάζα σε αλάτι.<br>β. Σε 100 g γιαουρτιού περιέχονται 2 g λιπαρά.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Να συμπληρώσετε τα κενά με τις κατάλληλες λέξεις ή αριθμούς: α. Σε 100 g υδατικού διαλύματος φρουκτόζης 20% μάζα προς μάζα περιέχονται ... g φρουκτόζης και ... g νερό. β. Μεταξύ δύο υδατικών διαλυμάτων ζάχαρης με περιεκτικότητες 10% μάζα προς όγκο και 30% μάζα προς όγκο πιο πυκνό είναι ...",
    "a": "<span class='text-purple-300 font-bold'>α. 20 g φρουκτόζης και 80 g νερό.<br>β. το διάλυμα που έχει περιεκτικότητα 30% μάζα προς όγκο.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "α. Τι πληροφορίες μας δίνει η έκφραση: «Ο χυμός έχει περιεκτικότητα 0,2% μάζα προς όγκο σε κιτρικό οξύ;» β. Το πλήρες γάλα έχει περιεκτικότητα σε λιπαρά 15% μάζα προς όγκο. Πώς θα χαρακτηρίζατε ένα γάλα, πλήρες ή light, αν ξέρατε ότι περιέχει 6 g λιπαρών σε 300 mL;",
    "a": "<span class='text-purple-300 font-bold'>α. Σε 100 mL χυμού περιέχονται 0,2 g κιτρικού οξέος.<br>β. Σε 100 mL πλήρες γάλα περιέχονται 15 g λιπαρά. Σε 300 mL πλήρες γάλα θα έπρεπε να περιέχονται 45 g λιπαρά. Το γάλα αν ήταν πλήρες θα είχε 45 g λιπαρά στα 300 mL. Επομένως θα χαρακτηρίζαμε το γάλα ως light (αφού έχει μόνο 6 g).</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Να αντιστοιχίσετε καθένα από τα διαλύματα της στήλης Α στην ποσότητα διαλυμένης ουσίας που περιέχεται σε 200 mL του στη στήλη Β.",
    "a": "\n    <span class='text-purple-300 font-bold'>Ο συμπληρωμένος πίνακας αντιστοίχισης είναι:</span>\n    <div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n      <thead class='bg-white/10'>\n        <tr class='border-b border-white/20'>\n          <th class='py-3 px-4 font-semibold'>Περιεκτικότητα (Στήλη Α)</th>\n          <th class='py-3 px-4 font-semibold'>Ποσότητα σε 200 mL (Στήλη Β)</th>\n          <th class='py-3 px-4 font-semibold text-emerald-400'>Σωστή Αντιστοίχιση</th>\n        </tr>\n      </thead>\n      <tbody class=\"divide-y divide-white/10\">\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>1. 20% μάζα προς όγκο</td><td class='py-3 px-4'>80 g</td><td class='py-3 px-4 text-emerald-400'>1 → 40 g</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>2. 10% μάζα προς όγκο</td><td class='py-3 px-4'>40 g</td><td class='py-3 px-4 text-emerald-400'>2 → 20 g</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>3. 15% μάζα προς όγκο</td><td class='py-3 px-4'>60 g</td><td class='py-3 px-4 text-emerald-400'>3 → 30 g</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>4. 30% μάζα προς όγκο</td><td class='py-3 px-4'>30 g</td><td class='py-3 px-4 text-emerald-400'>4 → 60 g</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>5. 40% μάζα προς όγκο</td><td class='py-3 px-4'>20 g</td><td class='py-3 px-4 text-emerald-400'>5 → 80 g</td></tr>\n      </tbody>\n    </table></div>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Στο αλκοτέστ μετράται η ποσότητα του οινοπνεύματος στο αίμα του οδηγού. Το προτεινόμενο επιτρεπτό όριο στην Ευρωπαϊκή Ένωση είναι 0,2 g/L αίματος. Ποια είναι η μέγιστη επιτρεπτή % μάζα προς όγκο περιεκτικότητα του αίματος σε οινόπνευμα;",
    "a": "<span class='text-purple-300 font-bold'>1 L αντιστοιχεί σε 1000 mL. Σε 1000 mL αίματος περιέχονται 0,2 g οινόπνευμα. Οπότε στα 100 mL αίματος περιέχονται x g. Υπολογίζοντας, x = 0,02 g. Οπότε η μέγιστη περιεκτικότητα του αίματος σε οινόπνευμα είναι 0,02% μάζα προς όγκο.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Η βότκα που έχει περιεκτικότητα σε αλκοόλ 40% όγκο προς όγκο (%v/v) ή η μπίρα που έχει περιεκτικότητα σε αλκοόλ 5,5% όγκο προς όγκο (%v/v) είναι πιο βλαβερή για τον οργανισμό, αν καταναλωθούν ίσες ποσότητες; Να δικαιολογήσετε την απάντησή σας.",
    "a": "<span class='text-purple-300 font-bold'>Η βότκα έχει μεγαλύτερη περιεκτικότητα σε αλκοόλ οπότε είναι πιο βλαβερή για τον οργανισμό.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "α. Τι σημαίνει η έκφραση: Στον ατμοσφαιρικό αέρα η μέγιστη επιτρεπτή περιεκτικότητα σε μονοξείδιο του άνθρακα είναι 0,01 g μονοξειδίου του άνθρακα σε 1 m<sup>3</sup> αέρα; β. Ποια θα είναι η % μάζα προς όγκο περιεκτικότητα του αέρα σε μονοξείδιο του άνθρακα στο όριο συναγερμού;",
    "a": "<span class='text-purple-300 font-bold'>α. 1 m<sup>3</sup> αντιστοιχεί σε 1000 L αέρα. Οπότε σε 1000 L αέρα η μέγιστη επιτρεπτή ποσότητα σε μονοξείδιο του άνθρακα είναι 0,01 g.<br>β. Σε 1000 L αέρα περιέχονται 0,01 g. Σε 0,1 L (που είναι 100 mL) αέρα περιέχονται x g. Υπολογίζοντας, x = 0,000001 g. Οπότε η περιεκτικότητα του αέρα σε μονοξείδιο του άνθρακα είναι 0,000001% μάζα προς όγκο.</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "Να συμπληρώσετε τα κενά με τις κατάλληλες λέξεις: α. Το οινόπνευμα του εμπορίου περιέχει νερό και αιθανόλη με περιεκτικότητα σε αιθανόλη 95% όγκο προς όγκο (%v/v). Σε 1 μπουκάλι οινοπνεύματος του εμπορίου που περιέχει 200 mL περιέχονται ... mL οινόπνευμα. β. Ο ατμοσφαιρικός αέρας έχει περιεκτικότητα σε οξυγόνο 21% όγκο προς όγκο, 78% σε άζωτο και 1% σε διάφορα άλλα αέρια. Σε 100 cm<sup>3</sup> αέρα περιέχονται ... cm<sup>3</sup> οξυγόνο και ... cm<sup>3</sup> άζωτο.",
    "a": "<span class='text-purple-300 font-bold'>α. 190 mL οινόπνευμα.<br>β. 21 cm<sup>3</sup> οξυγόνο και 78 cm<sup>3</sup> άζωτο.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Να αναφέρετε συνοπτικά τις απόψεις των αρχαίων Ελλήνων φιλοσόφων (Δημόκριτος, Αριστοτέλης) και του Lavoisier σχετικά με τη δομή της ύλης, όπως περιγράφονται στο εννοιολογικό σχήμα.",
    "a": "<span class='text-purple-300 font-bold'>\n    <ul class='list-disc ml-5 mt-2'>\n      <li class='mb-2'>Ο Δημόκριτος πίστευε ότι τα υλικά σώματα αποτελούνται από άτομα τα οποία είναι άφθαρτα, άτμητα και αναλλοίωτα. Ο συνδυασμός τους σχηματίζει τα ουράνια σώματα. Η θεωρία του δεν έγινε αποδεκτή από τους συγχρόνους του.</li>\n      <li class='mb-2'>Ο Αριστοτέλης πίστευε ότι τα υλικά σώματα προέρχονται από τον συνδυασμό 4 στοιχείων τα οποία είναι αέρας, φωτιά, γη, νερό και του αιθέρα. Η θεωρία του έγινε αποδεκτή, επηρέασε τους αλχημιστές (στην προσπάθειά τους για τη φιλοσοφική λίθο και το ελιξίριο της ζωής) και καταρρίφθηκε αργότερα.</li>\n      <li class='mb-2'>Ο Lavoisier απέδειξε ότι το νερό σχηματίζεται από H<sub>2</sub> και O<sub>2</sub>, δηλαδή είναι χημική ένωση.</li>\n    </ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "α. Πότε ένα υλικό σώμα χαρακτηρίζεται καθαρή ουσία; β. Πότε μία χημική ουσία χαρακτηρίζεται χημικό στοιχείο; (Δώστε 2 παραδείγματα) γ. Πότε μία χημική ουσία χαρακτηρίζεται χημική ένωση; (Δώστε 2 παραδείγματα).",
    "a": "<span class='text-purple-300 font-bold'>\n    <ul class='list-disc ml-5 mt-2'>\n      <li class='mb-2'>α. Καθαρές ουσίες ονομάζονται οι ουσίες που έχουν απόλυτα καθορισμένη σύσταση.</li>\n      <li class='mb-2'>β. Χημικό στοιχείο είναι μία ουσία που δεν μπορεί να διασπαστεί σε άλλες απλούστερες. Χαρακτηριστικά παραδείγματα είναι το υδρογόνο και το οξυγόνο.</li>\n      <li class='mb-2'>γ. Χημική ένωση είναι μία ουσία που μπορεί να διασπαστεί σε άλλες απλούστερες, έχει σταθερή σύσταση και έχει καθορισμένες φυσικές σταθερές. Χαρακτηριστικά παραδείγματα είναι το νερό και η ζάχαρη.</li>\n    </ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "α. Πώς κατέληξε ο Lavoisier στο συμπέρασμα ότι το νερό είναι μία χημική ένωση; β. Ποιες πειραματικές μεθόδους χρησιμοποιούμε για να διαπιστώσουμε τα στοιχεία του νερού; γ. Αν παράγονται 2 L αερίου Α και 4 L αερίου Β, ποιο είναι το καθένα; δ. Αν παράγονται 4 g οξυγόνου, πόσα g υδρογόνου παράγονται και πόσα g νερού διασπάστηκαν;",
    "a": "<span class='text-purple-300 font-bold'>\n    <ul class='list-disc ml-5 mt-2'>\n      <li class='mb-2'>α. Ο Lavoisier παρασκεύασε νερό από υδρογόνο και οξυγόνο, δηλαδή από απλούστερες χημικές ουσίες.</li>\n      <li class='mb-2'>β. Για το οξυγόνο βάζουμε μια μισοσβησμένη παρασχίδα στον σωλήνα και παρατηρούμε ότι αναφλέγεται. Για το υδρογόνο πλησιάζουμε αναμμένη παρασχίδα στις φυσαλίδες και ακούμε χαρακτηριστικό κρότο (καίγεται με έντονη φλόγα).</li>\n      <li class='mb-2'>γ. Ο όγκος του υδρογόνου είναι διπλάσιος από τον όγκο του οξυγόνου. Επομένως το αέριο Α (2 L) είναι το οξυγόνο και το Β (4 L) το υδρογόνο.</li>\n      <li class='mb-2'>δ. Η μάζα του οξυγόνου είναι οκταπλάσια του υδρογόνου. Άρα για 4 g οξυγόνου παράγονται 0,5 g υδρογόνου. Συνολικά διασπάστηκαν 4,5 g νερού.</li>\n    </ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Η κρυσταλλική ζάχαρη είναι μία χημική ένωση με σημείο τήξης περίπου 170°C. Πώς θα βρείτε αν ένα δείγμα είναι νοθευμένο; Αν αρχίζει να λιώνει στους 160°C και η θερμοκρασία αυξάνεται σταδιακά, έχει προσμείξεις;",
    "a": "<span class='text-purple-300 font-bold'>Θα μπορούσαμε να θερμάνουμε το δείγμα ζάχαρης. Αν το σημείο τήξης του δείγματος είναι διαφορετικό από 170°C (όπως στην περίπτωση που λιώνει στους 160°C), τότε έχει προσμείξεις και είναι νοθευμένο.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Να συμπληρώσετε τα κενά: α. Τα μείγματα σε αντίθεση με τις χημικές ουσίες δεν έχουν ... β. Κατά τη διάρκεια του βρασμού του απιοντισμένου νερού η θερμοκρασία ... γιατί το νερό είναι ...",
    "a": "<span class='text-purple-300 font-bold'>α. καθορισμένες φυσικές σταθερές. <br>β. διατηρείται σταθερή, γιατί το νερό είναι καθαρή ουσία.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Ο Ίον έχει μάζα 75 kg και το 66% της μάζας του είναι νερό (δηλαδή 49,5 kg νερό). Με δεδομένο ότι η αναλογία μαζών υδρογόνου - οξυγόνου στο νερό είναι σταθερή και ίση με 1/8, υπολογίστε τις μάζες τους.",
    "a": "<span class='text-purple-300 font-bold'>Η μάζα του υδρογόνου στο νερό στο σώμα του Ίονα είναι 5,5 kg και η μάζα του οξυγόνου είναι 44 kg.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Δίνεται διάγραμμα θέρμανσης υγρών Α και Β. Τη χρονική στιγμή t<sub>1</sub> αρχίζει να βράζει το Α και τη χρονική στιγμή t<sub>2</sub> το Β. Στο διάγραμμα, το σώμα Β διατηρεί σταθερή θερμοκρασία μετά την t<sub>2</sub>, ενώ το Α όχι. Είναι κάποιο από τα δύο χημική ουσία;",
    "a": "<span class='text-purple-300 font-bold'>Καθαρή ουσία είναι μόνο το σώμα Β, καθώς μετά τη χρονική στιγμή t2 (όπου αρχίζει ο βρασμός του) παρατηρούμε ότι η θερμοκρασία παραμένει σταθερή.</span>"
  },
  {
    "chapterId": "chapter-5",
    "q": "Ποιο είναι το βασικό δομικό υλικό των υλικών σωμάτων;",
    "a": "<span class='text-purple-300 font-bold'>Το βασικό δομικό υλικό των υλικών σωμάτων είναι το άτομο.</span>"
  },
  {
    "chapterId": "chapter-5",
    "q": "Ποιες ομοιότητες και ποιες διαφορές παρουσιάζουν οι ατομικές θεωρίες του Δημόκριτου και του Dalton;",
    "a": "<span class='text-purple-300 font-bold'>Οι ομοιότητες είναι ότι πίστευαν ότι η ύλη δεν τέμνεται επ' άπειρο αλλά αποτελείται από πολύ μικρά σωματίδια, τα οποία είναι άφθαρτα και άτμητα. Επίσης υποστήριξαν ότι τα άτομα έχουν συγκεκριμένα χαρακτηριστικά και οι διαφορετικές ουσίες αποτελούνται από διαφορετικά είδη ατόμων. Η διαφορά είναι ότι ο Δημόκριτος δεν ανέπτυξε συγκεκριμένη θεωρία για τον τρόπο σύνδεσης των ατόμων, ενώ ο Dalton υποστήριξε ότι τα άτομα συνδυάζονται με συγκεκριμένες αναλογίες σχηματίζοντας χημικές ενώσεις.</span>"
  },
  {
    "chapterId": "chapter-5",
    "q": "Να συμπληρώσετε τα κενά: α. Τα άτομα συνδυάζονται σε καθορισμένη ... και σχηματίζουν ενώσεις. Τα μόρια από όμοια άτομα είναι ... ενώ από διαφορετικά είναι ... β. ... ονομάζεται ο αριθμός ατόμων που σχηματίζουν το μόριο του στοιχείου.",
    "a": "<span class='text-purple-300 font-bold'>α. αναλογία. Τα μόρια που σχηματίζονται από τον συνδυασμό όμοιων ατόμων είναι μόρια στοιχείων ενώ αυτά που σχηματίζονται από τον συνδυασμό διαφορετικών ατόμων είναι μόρια χημικών ενώσεων. <br>β. Ατομικότητα ονομάζεται ο αριθμός ατόμων που σχηματίζουν το μόριο του στοιχείου.</span>"
  },
  {
    "chapterId": "chapter-5",
    "q": "Παρατηρώντας ένα προσομοίωμα ακετόνης (με άτομα άνθρακα, οξυγόνου και υδρογόνου): α. Το προσομοίωμα αναπαριστά μόριο χημικής ένωσης ή χημικού στοιχείου; β. Από ποια άτομα αποτελείται;",
    "a": "<span class='text-purple-300 font-bold'>α. Το προσομοίωμα αναπαριστά μόριο χημικής ένωσης καθώς αποτελείται από άτομα διαφορετικών στοιχείων. <br>β. Το μόριο της ακετόνης αποτελείται από 3 άτομα άνθρακα, 6 άτομα υδρογόνου και 1 άτομο οξυγόνου.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "α. Πώς συμβολίζονται τα άτομα των χημικών στοιχείων; β. Πώς συμβολίζονται τα μόρια των χημικών στοιχείων;",
    "a": "<span class='text-purple-300 font-bold'>α. Συμβολίζονται με ένα κεφαλαίο γράμμα που σε μερικές περιπτώσεις συνοδεύεται από ένα πεζό (αρχικά του ονόματός τους). <br>β. Από το σύμβολο του ατόμου με έναν αριθμό ως δείκτη δεξιά ο οποίος δηλώνει τον αριθμό των ατόμων (Π.χ. H<sub>2</sub>).</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Πώς συμβολίζονται τα μόρια των χημικών ενώσεων; Να γράψετε ένα παράδειγμα.",
    "a": "<span class='text-purple-300 font-bold'>Με τα σύμβολα των ατόμων που το καθένα έχει έναν αριθμό ως δείκτη δεξιά, που δηλώνει τον αριθμό των ατόμων που μετέχουν στο μόριο της χημικής ένωσης. Π.χ. H<sub>2</sub>O.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Να συμπληρώσετε τα κενά: α. Ο μοριακός τύπος του νιτρικού οξέος: HNO<sub>3</sub> μας πληροφορεί ότι... β. Ο μοριακός τύπος της αιθανόλης: C<sub>2</sub>H<sub>6</sub>O μας πληροφορεί ότι...",
    "a": "<span class='text-purple-300 font-bold'>α. Το μόριο αποτελείται από 1 άτομο αζώτου, 1 άτομο υδρογόνου και 3 άτομα οξυγόνου. <br>β. Το μόριο αποτελείται από 1 άτομο οξυγόνου, 2 άτομα άνθρακα και 6 άτομα υδρογόνου.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Να γράψετε τον μοριακό τύπο για το Υδρόθειο (2 άτομα υδρογόνου, 1 άτομο θείου) και το Πεντοξείδιο του αζώτου (5 άτομα οξυγόνου, 2 άτομα αζώτου).",
    "a": "<span class='text-purple-300 font-bold'>Υδρόθειο: H<sub>2</sub>S. Πεντοξείδιο του αζώτου: N<sub>2</sub>O<sub>5</sub>.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Να συμπληρώσετε τον πίνακα με τα σύμβολα των στοιχείων (υδρογόνο, οξυγόνο, άζωτο, άνθρακας, χαλκός, σίδηρος, θείο, φωσφόρος, χλώριο, βρώμιο, ιώδιο, μαγνήσιο, κάλιο, νάτριο, υδράργυρος, αργίλιο, ασβέστιο, πυρίτιο).",
    "a": "\n    <span class='text-purple-300 font-bold'>Ο συμπληρωμένος πίνακας είναι:</span>\n    <div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n      <thead class='bg-white/10'>\n        <tr class='border-b border-white/20'>\n          <th class='py-3 px-4 font-semibold'>Στοιχείο</th>\n          <th class='py-3 px-4 font-semibold'>Σύμβολο</th>\n          <th class='py-3 px-4 font-semibold'>Στοιχείο</th>\n          <th class='py-3 px-4 font-semibold'>Σύμβολο</th>\n        </tr>\n      </thead>\n      <tbody class=\"divide-y divide-white/10\">\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>υδρογόνο</td><td class='py-3 px-4 text-emerald-400'>H</td><td class='py-3 px-4'>κάλιο</td><td class='py-3 px-4 text-emerald-400'>K</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>οξυγόνο</td><td class='py-3 px-4 text-emerald-400'>O</td><td class='py-3 px-4'>νάτριο</td><td class='py-3 px-4 text-emerald-400'>Na</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>άζωτο</td><td class='py-3 px-4 text-emerald-400'>N</td><td class='py-3 px-4'>υδράργυρος</td><td class='py-3 px-4 text-emerald-400'>Hg</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>άνθρακας</td><td class='py-3 px-4 text-emerald-400'>C</td><td class='py-3 px-4'>αργίλιο</td><td class='py-3 px-4 text-emerald-400'>Al</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>χαλκός</td><td class='py-3 px-4 text-emerald-400'>Cu</td><td class='py-3 px-4'>ασβέστιο</td><td class='py-3 px-4 text-emerald-400'>Ca</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>σίδηρος</td><td class='py-3 px-4 text-emerald-400'>Fe</td><td class='py-3 px-4'>πυρίτιο</td><td class='py-3 px-4 text-emerald-400'>Si</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>θείο</td><td class='py-3 px-4 text-emerald-400'>S</td><td class='py-3 px-4'>χλώριο</td><td class='py-3 px-4 text-emerald-400'>Cl</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>φωσφόρος</td><td class='py-3 px-4 text-emerald-400'>P</td><td class='py-3 px-4'>βρώμιο</td><td class='py-3 px-4 text-emerald-400'>Br</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ιώδιο</td><td class='py-3 px-4 text-emerald-400'>I</td><td class='py-3 px-4'>μαγνήσιο</td><td class='py-3 px-4 text-emerald-400'>Mg</td></tr>\n      </tbody>\n    </table></div>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Να συμπληρώσετε τον πίνακα δίνοντας το όνομα και τον χαρακτηρισμό (χημική ένωση ή διατομικό/τριατομικό/κτλ στοιχείο) για τους τύπους: H<sub>2</sub>O, HCl, CO, N<sub>2</sub>, CO<sub>2</sub>, NH<sub>3</sub>, S<sub>6</sub>, P<sub>4</sub>, O<sub>3</sub>, Cl<sub>2</sub>.",
    "a": "\n    <span class='text-purple-300 font-bold'>Ο συμπληρωμένος πίνακας είναι:</span>\n    <div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n      <thead class='bg-white/10'>\n        <tr class='border-b border-white/20'>\n          <th class='py-3 px-4 font-semibold'>Χημικός τύπος</th>\n          <th class='py-3 px-4 font-semibold'>Όνομα</th>\n          <th class='py-3 px-4 font-semibold'>Χαρακτηρισμός</th>\n        </tr>\n      </thead>\n      <tbody class=\"divide-y divide-white/10\">\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>H<sub>2</sub>O</td><td class='py-3 px-4'>Νερό</td><td class='py-3 px-4 text-emerald-400'>χημική ένωση</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>HCl</td><td class='py-3 px-4'>Υδροχλώριο</td><td class='py-3 px-4 text-emerald-400'>χημική ένωση</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>CO</td><td class='py-3 px-4'>Μονοξείδιο του άνθρακα</td><td class='py-3 px-4 text-emerald-400'>χημική ένωση</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>N<sub>2</sub></td><td class='py-3 px-4'>Άζωτο</td><td class='py-3 px-4 text-blue-400'>διατομικό στοιχείο</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>CO<sub>2</sub></td><td class='py-3 px-4'>Διοξείδιο του άνθρακα</td><td class='py-3 px-4 text-emerald-400'>χημική ένωση</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>NH<sub>3</sub></td><td class='py-3 px-4'>Αμμωνία</td><td class='py-3 px-4 text-emerald-400'>χημική ένωση</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>S<sub>6</sub></td><td class='py-3 px-4'>Θείο</td><td class='py-3 px-4 text-blue-400'>εξατομικό στοιχείο</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>P<sub>4</sub></td><td class='py-3 px-4'>Φωσφόρος</td><td class='py-3 px-4 text-blue-400'>τετρατομικό στοιχείο</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>O<sub>3</sub></td><td class='py-3 px-4'>Όζον</td><td class='py-3 px-4 text-blue-400'>τριατομικό στοιχείο</td></tr>\n        <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Cl<sub>2</sub></td><td class='py-3 px-4'>Χλώριο</td><td class='py-3 px-4 text-blue-400'>διατομικό στοιχείο</td></tr>\n      </tbody>\n    </table></div>"
  },
  {
    "chapterId": "chapter-4",
    "q": "9. Να συμπληρώσετε τα ονόματα των χημικών στοιχείων και των χημικών ενώσεων των οποίων δίνονται οι χημικοί τύποι και να τα χαρακτηρίσετε στην τελευταία στήλη ως μονοατομικό (διατομικό...) στοιχείο ή χημική ένωση.\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'>\n      <th class='py-3 px-4 font-semibold'>Χημικός τύπος</th>\n      <th class='py-3 px-4 font-semibold'>Όνομα</th>\n      <th class='py-3 px-4 font-semibold'>Χαρακτηρισμός</th>\n    </tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>H<sub>2</sub>O</td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>HCl</td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>CO</td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>N<sub>2</sub></td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>CO<sub>2</sub></td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>NH<sub>3</sub></td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>S<sub>6</sub></td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>P<sub>4</sub></td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>O<sub>3</sub></td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Cl<sub>2</sub></td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n  </tbody>\n</table></div>",
    "a": "<span class='text-purple-300 font-bold'>\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'>\n      <th class='py-3 px-4 font-semibold'>Χημικός τύπος</th>\n      <th class='py-3 px-4 font-semibold'>Όνομα</th>\n      <th class='py-3 px-4 font-semibold'>Χαρακτηρισμός</th>\n    </tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>H<sub>2</sub>O</td><td class='py-3 px-4'>Νερό</td><td class='py-3 px-4 text-emerald-400'>χημική ένωση</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>HCl</td><td class='py-3 px-4'>Υδροχλώριο</td><td class='py-3 px-4 text-emerald-400'>χημική ένωση</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>CO</td><td class='py-3 px-4'>Μονοξείδιο του άνθρακα</td><td class='py-3 px-4 text-emerald-400'>χημική ένωση</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>N<sub>2</sub></td><td class='py-3 px-4'>Άζωτο</td><td class='py-3 px-4 text-blue-400'>διατομικό στοιχείο</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>CO<sub>2</sub></td><td class='py-3 px-4'>Διοξείδιο του άνθρακα</td><td class='py-3 px-4 text-emerald-400'>χημική ένωση</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>NH<sub>3</sub></td><td class='py-3 px-4'>Αμμωνία</td><td class='py-3 px-4 text-emerald-400'>χημική ένωση</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>S<sub>6</sub></td><td class='py-3 px-4'>Θείο</td><td class='py-3 px-4 text-blue-400'>εξατομικό στοιχείο</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>P<sub>4</sub></td><td class='py-3 px-4'>Φωσφόρος</td><td class='py-3 px-4 text-blue-400'>τετρατομικό στοιχείο</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>O<sub>3</sub></td><td class='py-3 px-4'>Όζον</td><td class='py-3 px-4 text-blue-400'>τριατομικό στοιχείο</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Cl<sub>2</sub></td><td class='py-3 px-4'>Χλώριο</td><td class='py-3 px-4 text-blue-400'>διατομικό στοιχείο</td></tr>\n  </tbody>\n</table></div>\n</span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "2. Να αναφέρετε τα δομικά σωματίδια ενός ατόμου και τα χαρακτηριστικά τους ως προς το φορτίο, τη μάζα και τη θέση τους στο άτομο.",
    "a": "<span class='text-purple-300 font-bold'>Τα δομικά σωματίδια ενός ατόμου είναι:\n<ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>1. Τα πρωτόνια (p) που είναι θετικά φορτισμένα σωματίδια και έχουν το στοιχειώδες θετικό φορτίο (+e).</li>\n  <li class='mb-2'>2. Τα νετρόνια (n) που είναι ηλεκτρικά ουδέτερα σωματίδια με μάζα περίπου ίση με τη μάζα του πρωτονίου.</li>\n  <li class='mb-2'>3. Τα ηλεκτρόνια (e) που είναι αρνητικά φορτισμένα σωματίδια και έχουν το στοιχειώδες αρνητικό φορτίο (-e), ενώ έχουν αμελητέα μάζα.<br>Η μάζα του ηλεκτρονίου είναι 1.836 φορές μικρότερη από τη μάζα του πρωτονίου.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "3. α. Τι εκφράζει και πώς συμβολίζεται ο ατομικός αριθμός;<br>β. Τι εκφράζει και πώς συμβολίζεται ο μαζικός αριθμός;",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Ατομικός αριθμός (Z) ονομάζεται ο αριθμός των πρωτονίων που περιέχονται στον πυρήνα του ατόμου ενός στοιχείου.</li>\n  <li class='mb-2'>β. Μαζικός αριθμός (A) ονομάζεται το άθροισμα του αριθμού των πρωτονίων και του αριθμού των νετρονίων που περιέχονται στον πυρήνα του ατόμου ενός στοιχείου.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "4. Να συμπληρώσετε τα κενά του παρακάτω πίνακα.\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'>\n      <th class='py-3 px-4 font-semibold'>Στοιχείο</th><th class='py-3 px-4 font-semibold'>Σύμβολο</th><th class='py-3 px-4 font-semibold'>Ατομικός αριθμός</th><th class='py-3 px-4 font-semibold'>Μαζικός αριθμός</th><th class='py-3 px-4 font-semibold'>Αριθμός πρωτονίων</th><th class='py-3 px-4 font-semibold'>Αριθμός ηλεκτρονίων</th><th class='py-3 px-4 font-semibold'>Αριθμός νετρονίων</th>\n    </tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΧΛΩΡΙΟ</td><td class='py-3 px-4'>Cl</td><td class='py-3 px-4'></td><td class='py-3 px-4'>35</td><td class='py-3 px-4'>17</td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n  </tbody>\n</table></div>",
    "a": "<span class='text-purple-300 font-bold'>\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'>\n      <th class='py-3 px-4 font-semibold'>Στοιχείο</th><th class='py-3 px-4 font-semibold'>Σύμβολο</th><th class='py-3 px-4 font-semibold'>Ατομικός αριθμός</th><th class='py-3 px-4 font-semibold'>Μαζικός αριθμός</th><th class='py-3 px-4 font-semibold'>Αριθμός πρωτονίων</th><th class='py-3 px-4 font-semibold'>Αριθμός ηλεκτρονίων</th><th class='py-3 px-4 font-semibold'>Αριθμός νετρονίων</th>\n    </tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΧΛΩΡΙΟ</td><td class='py-3 px-4 text-emerald-400'>Cl</td><td class='py-3 px-4'>17</td><td class='py-3 px-4'>35</td><td class='py-3 px-4'>17</td><td class='py-3 px-4'>17</td><td class='py-3 px-4'>18</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΑΖΩΤΟ</td><td class='py-3 px-4 text-emerald-400'>N</td><td class='py-3 px-4'>7</td><td class='py-3 px-4'>14</td><td class='py-3 px-4'>7</td><td class='py-3 px-4'>7</td><td class='py-3 px-4'>7</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΜΑΓΝΗΣΙΟ</td><td class='py-3 px-4 text-emerald-400'>Mg</td><td class='py-3 px-4'>12</td><td class='py-3 px-4'>24</td><td class='py-3 px-4'>12</td><td class='py-3 px-4'>12</td><td class='py-3 px-4'>12</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΑΡΓΙΛΙΟ</td><td class='py-3 px-4 text-emerald-400'>Al</td><td class='py-3 px-4'>13</td><td class='py-3 px-4'>27</td><td class='py-3 px-4'>13</td><td class='py-3 px-4'>13</td><td class='py-3 px-4'>14</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΠΥΡΙΤΙΟ</td><td class='py-3 px-4 text-emerald-400'>Si</td><td class='py-3 px-4'>14</td><td class='py-3 px-4'>28</td><td class='py-3 px-4'>14</td><td class='py-3 px-4'>14</td><td class='py-3 px-4'>14</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΣΙΔΗΡΟΣ</td><td class='py-3 px-4 text-emerald-400'>Fe</td><td class='py-3 px-4'>26</td><td class='py-3 px-4'>56</td><td class='py-3 px-4'>26</td><td class='py-3 px-4'>26</td><td class='py-3 px-4'>30</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΒΡΩΜΙΟ</td><td class='py-3 px-4 text-emerald-400'>Br</td><td class='py-3 px-4'>35</td><td class='py-3 px-4'>80</td><td class='py-3 px-4'>35</td><td class='py-3 px-4'>35</td><td class='py-3 px-4'>45</td></tr>\n  </tbody>\n</table></div>\n</span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "5. Δίνεται ο ατομικός και ο μαζικός αριθμός των στοιχείων:<br>1. άζωτο έχει Z=7 και A=14, 2. χλώριο έχει Z=17 και A=35, 3. κάλιο έχει Z=19 και A=39.<br>α. Να περιγράψετε τη δομή του ατόμου κάθε στοιχείου.<br>β. Να κάνετε την κατανομή των ηλεκτρονίων του σε στιβάδες.",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>1. α. Το άτομο του αζώτου περιέχει: 7 πρωτόνια, 7 ηλεκτρόνια και 7 νετρόνια.<br>β. K(2) - L(5)</li>\n  <li class='mb-2'>2. α. Το άτομο του χλωρίου περιέχει: 17 πρωτόνια, 17 ηλεκτρόνια και 18 νετρόνια.<br>β. K(2) - L(8) - M(7)</li>\n  <li class='mb-2'>3. α. Το άτομο του καλίου περιέχει: 19 πρωτόνια, 19 ηλεκτρόνια και 20 νετρόνια.<br>β. K(2) - L(8) - M(8) - N(1)</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "6. Να συμπληρώσετε τον παρακάτω πίνακα στιβάδων κάνοντας την κατανομή των ηλεκτρονίων για το στοιχείο της κάθε γραμμής.<br>α. Ποια στοιχεία του πίνακα έχουν συμπληρωμένη την εξωτερική τους στιβάδα, δηλαδή είναι ευγενή αέρια;<br>β. Ποια στοιχεία του πίνακα έχουν την ίδια εξωτερική στιβάδα;<br>γ. Ποια στοιχεία του πίνακα έχουν 2 e στην εξωτερική στιβάδα;<br>δ. Ποια στοιχεία του πίνακα έχουν 6 e στην εξωτερική στιβάδα;",
    "a": "<span class='text-purple-300 font-bold'>\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'><th class='py-3 px-4 font-semibold'>ΣΤΟΙΧΕΙΟ</th><th class='py-3 px-4 font-semibold'>ΣΥΜΒΟΛΟ</th><th class='py-3 px-4 font-semibold'>ΑΤΟΜΙΚΟΣ ΑΡΙΘΜΟΣ</th><th class='py-3 px-4 font-semibold'>K L M N O P Q</th></tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΥΔΡΟΓΟΝΟ</td><td class='py-3 px-4 text-emerald-400'>H</td><td class='py-3 px-4'>1</td><td class='py-3 px-4'>1</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΝΕΟΝ</td><td class='py-3 px-4 text-emerald-400'>Ne</td><td class='py-3 px-4'>10</td><td class='py-3 px-4'>2 8</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΛΙΘΙΟ</td><td class='py-3 px-4 text-emerald-400'>Li</td><td class='py-3 px-4'>3</td><td class='py-3 px-4'>2 1</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΝΑΤΡΙΟ</td><td class='py-3 px-4 text-emerald-400'>Na</td><td class='py-3 px-4'>11</td><td class='py-3 px-4'>2 8 1</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΜΑΓΝΗΣΙΟ</td><td class='py-3 px-4 text-emerald-400'>Mg</td><td class='py-3 px-4'>12</td><td class='py-3 px-4'>2 8 2</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΑΝΘΡΑΚΑΣ</td><td class='py-3 px-4 text-emerald-400'>C</td><td class='py-3 px-4'>6</td><td class='py-3 px-4'>2 4</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΑΖΩΤΟ</td><td class='py-3 px-4 text-emerald-400'>N</td><td class='py-3 px-4'>7</td><td class='py-3 px-4'>2 5</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΘΕΙΟ</td><td class='py-3 px-4 text-emerald-400'>S</td><td class='py-3 px-4'>16</td><td class='py-3 px-4'>2 8 6</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>ΧΛΩΡΙΟ</td><td class='py-3 px-4 text-emerald-400'>Cl</td><td class='py-3 px-4'>17</td><td class='py-3 px-4'>2 8 7</td></tr>\n  </tbody>\n</table></div>\n<ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Το στοιχείο Νέον έχει συμπληρωμένη την εξωτερική του στιβάδα με 8 ηλεκτρόνια.</li>\n  <li class='mb-2'>β. Τα στοιχεία Na, Mg, S και Cl έχουν εξωτερική στιβάδα την M. Τα στοιχεία Ne, Li, C, N έχουν εξωτερική στιβάδα την L.</li>\n  <li class='mb-2'>γ. 2 e στην εξωτερική στιβάδα έχει το στοιχείο Mg.</li>\n  <li class='mb-2'>δ. 6 e στην εξωτερική στιβάδα έχει το στοιχείο S.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "7. Να συμπληρώσετε τα e στις στιβάδες για τα άτομα 15P και 13Al.",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>15P: K(2) - L(8) - M(5)</li>\n  <li class='mb-2'>13Al: K(2) - L(8) - M(3)</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "8. Να δώσετε με συντομία τους ορισμούς των όρων:<br>α. Ιόν<br>β. Ιοντική ένωση",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Ιόν ονομάζεται το ηλεκτρικά φορτισμένο σωματίδιο το οποίο προκύπτει όταν ένα άτομο προσλάβει ή αποβάλλει ηλεκτρόνια.</li>\n  <li class='mb-2'>β. Ιοντική ένωση ονομάζεται η ένωση που δεν αποτελείται από μόρια, αλλά από θετικά και αρνητικά ιόντα τα οποία έλκονται σχηματίζοντας κρυστάλλους.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "9. Το άτομο του ψευδαργύρου έχει στον πυρήνα του συνολικά 65 νουκλεόνια και γύρω από τον πυρήνα περιφέρονται 30 ηλεκτρόνια. Να βρείτε τον ατομικό και μαζικό αριθμό, τα πρωτόνια, τα νετρόνια και να εξηγήσετε πώς δημιουργείται το ιόν του.",
    "a": "<span class='text-purple-300 font-bold'>Ο ψευδάργυρος έχει 30 πρωτόνια και 35 νετρόνια, έχει ατομικό αριθμό Z=30, μαζικό αριθμό A=65.<br>Το ιόν του ψευδαργύρου δημιουργείται με αποβολή 2 ηλεκτρονίων και έχει φορτίο +2.</span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "11. Να συμπληρώσετε τα κενά στους πίνακες των ατόμων (α) και των ιόντων (β).",
    "a": "<span class='text-purple-300 font-bold'>\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'><th class='py-3 px-4 font-semibold'>Στοιχείο</th><th class='py-3 px-4 font-semibold'>Ατομικός αριθμός</th><th class='py-3 px-4 font-semibold'>Μαζικός αριθμός</th><th class='py-3 px-4 font-semibold'>Αριθμός πρωτονίων</th><th class='py-3 px-4 font-semibold'>Αριθμός ηλεκτρονίων</th><th class='py-3 px-4 font-semibold'>Κατανομή σε στιβάδες</th><th class='py-3 px-4 font-semibold'>Αριθμός νετρονίων</th></tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4 text-emerald-400'>ΘΕΙΟ</td><td class='py-3 px-4'>16</td><td class='py-3 px-4'>32</td><td class='py-3 px-4'>16</td><td class='py-3 px-4'>16</td><td class='py-3 px-4'>K(2)-L(8)-M(6)</td><td class='py-3 px-4'>16</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4 text-emerald-400'>ΟΞΥΓΟΝΟ</td><td class='py-3 px-4'>8</td><td class='py-3 px-4'>16</td><td class='py-3 px-4'>8</td><td class='py-3 px-4'>8</td><td class='py-3 px-4'>K(2)-L(6)</td><td class='py-3 px-4'>8</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4 text-emerald-400'>ΑΝΘΡΑΚΑΣ</td><td class='py-3 px-4'>6</td><td class='py-3 px-4'>12</td><td class='py-3 px-4'>6</td><td class='py-3 px-4'>6</td><td class='py-3 px-4'>K(2)-L(4)</td><td class='py-3 px-4'>12</td></tr>\n  </tbody>\n</table></div>\n<br>\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'><th class='py-3 px-4 font-semibold'>ΙΟΝ</th><th class='py-3 px-4 font-semibold'>Ατομικός αριθμός</th><th class='py-3 px-4 font-semibold'>Αριθμός πρωτονίων</th><th class='py-3 px-4 font-semibold'>Αριθμός ηλεκτρονίων</th><th class='py-3 px-4 font-semibold'>Κατανομή σε στιβάδες</th><th class='py-3 px-4 font-semibold'>Φορτίο ιόντος</th></tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4 text-blue-400'>ΧΛΩΡΙΟΥ</td><td class='py-3 px-4'>17</td><td class='py-3 px-4'>17</td><td class='py-3 px-4'>18</td><td class='py-3 px-4'>K(2)-L(8)-M(8)</td><td class='py-3 px-4'>-1</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4 text-blue-400'>ΜΑΓΝΗΣΙΟΥ</td><td class='py-3 px-4'>12</td><td class='py-3 px-4'>12</td><td class='py-3 px-4'>10</td><td class='py-3 px-4'>K(2)-L(8)</td><td class='py-3 px-4'>+2</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4 text-blue-400'>ΚΑΛΙΟΥ</td><td class='py-3 px-4'>19</td><td class='py-3 px-4'>19</td><td class='py-3 px-4'>18</td><td class='py-3 px-4'>K(2)-L(8)-M(8)</td><td class='py-3 px-4'>+1</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4 text-blue-400'>ΘΕΙΟΥ</td><td class='py-3 px-4'>16</td><td class='py-3 px-4'>16</td><td class='py-3 px-4'>18</td><td class='py-3 px-4'>K(2)-L(8)-M(8)</td><td class='py-3 px-4'>-2</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4 text-blue-400'>ΟΞΥΓΟΝΟΥ</td><td class='py-3 px-4'>8</td><td class='py-3 px-4'>8</td><td class='py-3 px-4'>10</td><td class='py-3 px-4'>K(2)-L(8)</td><td class='py-3 px-4'>-2</td></tr>\n  </tbody>\n</table></div>\n</span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "12. Ο ατομικός αριθμός του φθορίου είναι 9 και ο μαζικός αριθμός 19. Να απαντήσετε στις ακόλουθες ερωτήσεις που αφορούν το άτομο και τα ιόντα του φθορίου και να δικαιολογήσετε τις απαντήσεις σας.<br>α. Πόσα πρωτόνια, πόσα νετρόνια και πόσα ηλεκτρόνια συγκροτούν το άτομο του φθορίου;<br>β. Πού βρίσκεται συγκεντρωμένη η μάζα του φθορίου και γιατί;<br>γ. Είναι σωστό ή λάθος ότι το μεγαλύτερο μέρος του ατόμου είναι κενό;<br>δ. Είναι σωστό ή λάθος ότι το άτομο είναι ηλεκτρικά ουδέτερο;<br>ε. Το άτομο του φθορίου μπορεί να προσλάβει ένα ηλεκτρόνιο. Σε τι θα μετατραπεί σε αυτή την περίπτωση;<br>στ. Τα ιόντα φθορίου σχηματίζουν με τα ιόντα νατρίου (+1) την ιοντική ένωση NaF. Ποια είναι τα δομικά σωματίδια της ένωσης;",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Το άτομο του φθορίου περιέχει 9 πρωτόνια, 10 νετρόνια και 9 ηλεκτρόνια.</li>\n  <li class='mb-2'>β. Η μάζα του ατόμου είναι συγκεντρωμένη στον πυρήνα του.</li>\n  <li class='mb-2'>γ. Η πρόταση είναι σωστή. Παρόλο που η μάζα του ατόμου είναι συγκεντρωμένη στον πυρήνα, ο όγκος του πυρήνα είναι ελάχιστος σε σχέση με τον συνολικό όγκο του ατόμου, με αποτέλεσμα το άτομο να αποτελείται κυρίως από κενό χώρο.</li>\n  <li class='mb-2'>δ. Η πρόταση είναι σωστή. Το άτομο είναι ηλεκτρικά ουδέτερο, γιατί ο αριθμός πρωτονίων στον πυρήνα είναι ίσος με τον αριθμό των ηλεκτρονίων που περιφέρονται γύρω από τον πυρήνα.</li>\n  <li class='mb-2'>ε. Σε ιόν (ανιόν) με φορτίο -1.</li>\n  <li class='mb-2'>στ. Τα δομικά σωματίδια της ένωσης NaF είναι το κατιόν νατρίου και το ανιόν φθορίου.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "18. Να συμπληρώσετε την ακροστιχίδα (Λύση).\n1. Ακέφαλο το μέρος του ατόμου που περιέχει συγκεντρωμένη σχεδόν όλη τη μάζα του.\n2. Τα θετικά φορτισμένα υποατομικά σωματίδια.\n3. Τόσα ηλεκτρόνια έχει ένα άτομο με Z=8.\n4. Ο αριθμός των πρωτονίων του πυρήνα.\n5. Τόσα ηλεκτρόνια έχει προσλάβει ένα ιόν που έχει φορτίο -3.\n6. Τα νετρόνια είναι σωματίδια ηλεκτρικά...\n7. Ο αριθμός που δηλώνει το άθροισμα πρωτονίων και νετρονίων του πυρήνα.\n8. Δημιουργείται όταν ένα άτομο προσλαμβάνει ή αποβάλλει ηλεκτρόνια.\n9. Δημιουργείται όταν ένα άτομο αποβάλλει ηλεκτρόνια.\n10. Δημιουργείται όταν ένα άτομο προσλαμβάνει ηλεκτρόνια.",
    "a": "<div class='overflow-x-auto my-4'><table class='border-collapse text-sm'><thead><tr><th class='py-2 px-3 text-left text-white/60 font-normal'>#</th><th class='py-2 px-3 text-left text-white/60 font-normal'>Απάντηση</th></tr></thead><tbody><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>1.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Π</span><span class='text-emerald-300'>ΥΡΗΝΑΣ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>2.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Π</span><span class='text-emerald-300'>ΡΩΤΟΝΙΑ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>3.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ο</span><span class='text-emerald-300'>ΚΤΩ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>4.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Α</span><span class='text-emerald-300'>ΤΟΜΙΚΟΣ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>5.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Τ</span><span class='text-emerald-300'>ΡΙΑ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>6.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ο</span><span class='text-emerald-300'>ΥΔΕΤΕΡΑ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>7.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Μ</span><span class='text-emerald-300'>ΑΖΙΚΟΣ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>8.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ι</span><span class='text-emerald-300'>ΟΝ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>9.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Κ</span><span class='text-emerald-300'>ΑΤΙΟΝ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>10.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Α</span><span class='text-emerald-300'>ΝΙΟΝ</span></td></tr></tbody></table></div><p class='mt-3 text-yellow-300 font-bold text-base'>Αρχικά: <span class='tracking-widest text-yellow-400 text-lg'>Π-Π-Ο-Α-Τ-Ο-Μ-Ι-Κ-Α</span></p>"
  },
  {
    "chapterId": "chapter-4",
    "q": "1. α. Πότε ένα φαινόμενο χαρακτηρίζεται χημική αντίδραση;<br>β. Να χαρακτηρίσετε τα σώματα της ακόλουθης χημικής εξίσωσης ως αντιδρώντα ή προϊόντα: υδροχλώριο + υδροξείδιο του νατρίου -> χλωρίδιο του νατρίου + νερό",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Χημική αντίδραση ή χημική μεταβολή ονομάζεται κάθε μεταβολή κατά την οποία από τις αρχικές ουσίες, οι οποίες ονομάζονται αντιδρώντα, σχηματίζονται νέες ουσίες με διαφορετικές ιδιότητες, οι οποίες ονομάζονται προϊόντα.</li>\n  <li class='mb-2'>β. Αντιδρώντα είναι το υδροχλώριο και το υδροξείδιο του νατρίου. Προϊόντα είναι το χλωρίδιο του νατρίου και το νερό.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "2. Να συμπληρώσετε τα κενά στον ακόλουθο πίνακα για τις χημικές αντιδράσεις.\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'><th class='py-3 px-4 font-semibold'>Περιγραφή</th><th class='py-3 px-4 font-semibold'>Χημική αντίδραση με λέξεις</th><th class='py-3 px-4 font-semibold'>Χημική εξίσωση</th></tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Το μαγνήσιο αντιδρά με το οξυγόνο και παράγει το οξείδιο του μαγνησίου</td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Ο άνθρακας αντιδρά με το οξυγόνο και παράγει διοξείδιο του άνθρακα</td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Ο σίδηρος αντιδρά με το οξυγόνο και παράγει οξείδιο του σιδήρου</td><td class='py-3 px-4'></td><td class='py-3 px-4'></td></tr>\n  </tbody>\n</table></div>",
    "a": "<span class='text-purple-300 font-bold'>\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'><th class='py-3 px-4 font-semibold'>Περιγραφή</th><th class='py-3 px-4 font-semibold'>Χημική αντίδραση με λέξεις</th><th class='py-3 px-4 font-semibold'>Χημική εξίσωση</th></tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Το μαγνήσιο αντιδρά με το οξυγόνο και παράγει το οξείδιο του μαγνησίου</td><td class='py-3 px-4'>μαγνήσιο + οξυγόνο -> οξείδιο του μαγνησίου</td><td class='py-3 px-4'>2Mg + O<sub>2</sub> -> 2MgO</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Ο άνθρακας αντιδρά με το οξυγόνο και παράγει διοξείδιο του άνθρακα</td><td class='py-3 px-4'>άνθρακας + οξυγόνο -> διοξείδιο του άνθρακα</td><td class='py-3 px-4'>C + O<sub>2</sub> -> CO<sub>2</sub></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Ο σίδηρος αντιδρά με το οξυγόνο και παράγει οξείδιο του σιδήρου</td><td class='py-3 px-4'>σίδηρος + οξυγόνο -> οξείδιο του σιδήρου</td><td class='py-3 px-4'>2Fe + O<sub>2</sub> -> 2FeO</td></tr>\n  </tbody>\n</table></div>\n</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "3. Να αντιστοιχίσετε καθεμία από τις ουσίες της στήλης Α στα αντιδρώντα από τα οποία μπορεί να παραχθεί στη στήλη Β.\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'><th class='py-3 px-4 font-semibold'>Στήλη Α</th><th class='py-3 px-4 font-semibold'>Στήλη Β</th></tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>1. NaCl</td><td class='py-3 px-4'>1. H<sub>2</sub> + I<sub>2</sub></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>2. CO<sub>2</sub></td><td class='py-3 px-4'>2. N<sub>2</sub> + O<sub>2</sub></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>3. N<sub>2</sub>O<sub>5</sub></td><td class='py-3 px-4'>3. Fe + O<sub>2</sub></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>4. Fe<sub>2</sub>O<sub>3</sub></td><td class='py-3 px-4'>4. Na + Cl<sub>2</sub></td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>5. HI</td><td class='py-3 px-4'>5. C + O<sub>2</sub></td></tr>\n  </tbody>\n</table></div>",
    "a": "<span class='text-purple-300 font-bold'>\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'><th class='py-3 px-4 font-semibold'>Στήλη Α</th><th class='py-3 px-4 font-semibold'>Στήλη Β (Απαντήσεις)</th></tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>1. NaCl</td><td class='py-3 px-4'>4. Na + Cl<sub>2</sub> (A1 -> B4)</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>2. CO<sub>2</sub></td><td class='py-3 px-4'>5. C + O<sub>2</sub> (A2 -> B5)</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>3. N<sub>2</sub>O<sub>5</sub></td><td class='py-3 px-4'>2. N<sub>2</sub> + O<sub>2</sub> (A3 -> B2)</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>4. Fe<sub>2</sub>O<sub>3</sub></td><td class='py-3 px-4'>3. Fe + O<sub>2</sub> (A4 -> B3)</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>5. HI</td><td class='py-3 px-4'>1. H<sub>2</sub> + I<sub>2</sub> (A5 -> B1)</td></tr>\n  </tbody>\n</table></div>\n</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "4. Να αναπαραστήσετε τις ακόλουθες χημικές εξισώσεις με σύμβολα και με λέξεις (όπου δίνονται προσομοιώματα) ή το αντίστροφο.<br>α. Με προσομοιώματα: Δύο μόρια υδροβρωμίου παράγουν ένα μόριο υδρογόνου και ένα μόριο βρωμίου.<br>β. Με προσομοιώματα: Ένα μόριο αζώτου αντιδρά με 3 μόρια υδρογόνου και παράγουν δύο μόρια αμμωνίας.<br>γ. Με λέξεις: Ένα άτομο άνθρακα αντιδρά με δύο μόρια υδρογόνου και παράγεται ένα μόριο μεθανίου.<br>δ. Με λέξεις: 1 μόριο αζώτου αντιδρά με δύο μόρια οξυγόνου και παράγουν 2 μόρια διοξειδίου του αζώτου.",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Με λέξεις: 2 μόρια υδροβρωμίου παράγουν ένα μόριο υδρογόνου και ένα μόριο βρωμίου.<br>Με χημική εξίσωση: 2HBr -> H<sub>2</sub> + Br<sub>2</sub></li>\n  <li class='mb-2'>β. Με λέξεις: Ένα μόριο αζώτου αντιδρά με 3 μόρια υδρογόνου και παράγουν δύο μόρια αμμωνίας.<br>Με χημική εξίσωση: N<sub>2</sub> + 3H<sub>2</sub> -> 2NH<sub>3</sub></li>\n  <li class='mb-2'>γ. Με λέξεις: Ένα άτομο άνθρακα αντιδρά με δύο μόρια υδρογόνου και παράγεται ένα μόριο μεθανίου.<br>Με χημική εξίσωση: C + 2H<sub>2</sub> -> CH<sub>4</sub></li>\n  <li class='mb-2'>δ. Με χημική εξίσωση: N<sub>2</sub>(g) + 2O<sub>2</sub>(g) -> 2NO<sub>2</sub>(g)<br>Με λέξεις: 1 μόριο αζώτου αντιδρά με δύο μόρια οξυγόνου και παράγουν 2 μόρια διοξειδίου του αζώτου.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "5. Οι χημικές εξισώσεις που ακολουθούν έχουν ένα λάθος ή μία παράλειψη. Να το εντοπίσετε και να γράψετε τη χημική εξίσωση συμπληρωμένη σωστά.<br>α. N<sub>2</sub> + 3H<sub>2</sub> -> 2NH<sub>3</sub><br>β. H<sub>2</sub>(g) + I<sub>2</sub>(g) -> HI(g)<br>γ. H<sub>2</sub>(g) + O<sub>2</sub>(g) -> H<sub>2</sub>O(l)<br>δ. H<sub>2</sub>(g) + Cl<sub>2</sub>(g) -> 2HI(g)<br>ε. C(s) + O(g) -> CO(g)",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Δεν έχουν αναγραφεί οι φυσικές καταστάσεις των ουσιών.<br>N<sub>2</sub>(g) + 3H<sub>2</sub>(g) -> 2NH<sub>3</sub>(g)</li>\n  <li class='mb-2'>β. Δεν ισχύει η αρχή διατήρησης των ατόμων (απαιτείται συντελεστής).<br>H<sub>2</sub>(g) + I<sub>2</sub>(g) -> 2HI(g)</li>\n  <li class='mb-2'>γ. Δεν ισχύει η αρχή διατήρησης των ατόμων.<br>2H<sub>2</sub>(g) + O<sub>2</sub>(g) -> 2H<sub>2</sub>O(l)</li>\n  <li class='mb-2'>δ. Το προϊόν έχει αναγραφεί λανθασμένα καθώς πρέπει να παράγεται HCl.<br>H<sub>2</sub>(g) + Cl<sub>2</sub>(g) -> 2HCl(g)</li>\n  <li class='mb-2'>ε. Το οξυγόνο είναι διατομικό στοιχείο.<br>2C(s) + O<sub>2</sub>(g) -> 2CO(g)</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "6. Να συμπληρώσετε τους συντελεστές στις ακόλουθες εξισώσεις, έτσι ώστε όσα άτομα κάθε στοιχείου υπάρχουν στο πρώτο μέλος, τόσα να υπάρχουν και στο δεύτερο.<br>α. N<sub>2</sub>(g) + H<sub>2</sub>(g) -> NH<sub>3</sub>(g)<br>β. N<sub>2</sub>(g) + O<sub>2</sub>(g) -> NO(g)<br>γ. CH<sub>4</sub>(g) + O<sub>2</sub>(g) -> CO<sub>2</sub>(g) + H<sub>2</sub>O(g)<br>δ. SO<sub>2</sub>(g) + O<sub>2</sub>(g) -> SO<sub>3</sub>(g)<br>ε. C<sub>4</sub>H<sub>10</sub>(g) + O<sub>2</sub>(g) -> CO<sub>2</sub>(g) + H<sub>2</sub>O(g)",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. N<sub>2</sub>(g) + 3H<sub>2</sub>(g) -> 2NH<sub>3</sub>(g)</li>\n  <li class='mb-2'>β. N<sub>2</sub>(g) + O<sub>2</sub>(g) -> 2NO(g)</li>\n  <li class='mb-2'>γ. CH<sub>4</sub>(g) + 2O<sub>2</sub>(g) -> CO<sub>2</sub>(g) + 2H<sub>2</sub>O(g)</li>\n  <li class='mb-2'>δ. 2SO<sub>2</sub>(g) + O<sub>2</sub>(g) -> 2SO<sub>3</sub>(g)</li>\n  <li class='mb-2'>ε. 2C<sub>4</sub>H<sub>10</sub>(g) + 13O<sub>2</sub>(g) -> 8CO<sub>2</sub>(g) + 10H<sub>2</sub>O(g)</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "7. Να συμπληρώσετε τους δείκτες στους χημικούς τύπους στις ακόλουθες εξισώσεις, έτσι ώστε όσα άτομα κάθε στοιχείου υπάρχουν στο πρώτο μέλος, τόσα να υπάρχουν και στο δεύτερο.<br>α. 4P(s) + 5O(g) -> 2P<sub>2</sub>O<sub>5</sub>(g)<br>β. N(g) + 2O(g) -> 2NO<sub>2</sub>(g)<br>γ. C<sub>4</sub>H(g) + 6O<sub>2</sub>(g) -> 4CO<sub>2</sub>(g) + 4H<sub>2</sub>O(g)<br>δ. 2SO(g) -> 2SO<sub>2</sub>(g) + O<sub>2</sub>(g)",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. 4P(s) + 5O<sub>2</sub>(g) -> 2P<sub>2</sub>O<sub>5</sub>(g)</li>\n  <li class='mb-2'>β. N<sub>2</sub>(g) + 2O<sub>2</sub>(g) -> 2NO<sub>2</sub>(g)</li>\n  <li class='mb-2'>γ. C<sub>4</sub>H<sub>8</sub>(g) + 6O<sub>2</sub>(g) -> 4CO<sub>2</sub>(g) + 4H<sub>2</sub>O(g)</li>\n  <li class='mb-2'>δ. 2SO<sub>3</sub>(g) -> 2SO<sub>2</sub>(g) + O<sub>2</sub>(g)</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "8. Να αντιστοιχίσετε τα σύμβολα της γλώσσας με τα σύμβολα της χημείας στα οποία πιστεύετε ότι αντιστοιχούν.\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'><th class='py-3 px-4 font-semibold'>Στήλη Α</th><th class='py-3 px-4 font-semibold'>Στήλη Β</th></tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>1. γράμμα</td><td class='py-3 px-4'>1. μοριακός τύπος</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>2. λέξη</td><td class='py-3 px-4'>2. χημική εξίσωση</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>3. πρόταση</td><td class='py-3 px-4'>3. σύμβολο ατόμου</td></tr>\n  </tbody>\n</table></div>",
    "a": "<span class='text-purple-300 font-bold'>\n<div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n  <thead class='bg-white/10'>\n    <tr class='border-b border-white/20'><th class='py-3 px-4 font-semibold'>Στήλη Α</th><th class='py-3 px-4 font-semibold'>Στήλη Β (Απαντήσεις)</th></tr>\n  </thead>\n  <tbody class=\"divide-y divide-white/10\">\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>1. γράμμα</td><td class='py-3 px-4'>3. σύμβολο ατόμου (Α1 - Β3)</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>2. λέξη</td><td class='py-3 px-4'>1. μοριακός τύπος (Α2 - Β1)</td></tr>\n    <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>3. πρόταση</td><td class='py-3 px-4'>2. χημική εξίσωση (Α3 - Β2)</td></tr>\n  </tbody>\n</table></div>\n</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "9. Να παρατηρήσετε την ακόλουθη χημική εξίσωση: 4HCl(g) + O<sub>2</sub>(g) -> 2Cl<sub>2</sub>(g) + 2H<sub>2</sub>O(g)<br>στη συνέχεια:<br>α. Να χαρακτηρίσετε όλες τις χημικές ουσίες ως αντιδρώντα ή προϊόντα.<br>β. Να την περιγράψετε με λόγια.<br>γ. Να εξηγήσετε γιατί μπροστά από ορισμένα σύμβολα μορίων υπάρχουν αριθμοί και τι σημαίνουν αυτοί.",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Αντιδρώντα: HCl, O<sub>2</sub>. Προϊόντα: Cl<sub>2</sub>, H<sub>2</sub>O.</li>\n  <li class='mb-2'>β. 4 μόρια υδροχλωρίου αντιδρούν με 1 μόριο οξυγόνου και παράγουν 2 μόρια χλωρίου και 2 μόρια νερού.</li>\n  <li class='mb-2'>γ. Οι αριθμοί μπροστά από τους τύπους ονομάζονται συντελεστές και δηλώνουν τον αριθμό των μορίων που συμμετέχουν στην αντίδραση, ώστε να ισχύει η αρχή διατήρησης της μάζας (δηλαδή ο αριθμός των ατόμων κάθε στοιχείου στα αντιδρώντα να ισούται με τον αριθμό τους στα προϊόντα).</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "11. α. Ποιες αντιδράσεις χαρακτηρίζονται εξώθερμες;<br>β. Να αναφέρετε ένα παράδειγμα εξώθερμης αντίδρασης που σας είναι γνωστό από την καθημερινή σας ζωή.",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Εξώθερμες ονομάζονται οι αντιδράσεις κατά τις οποίες ελευθερώνεται ενέργεια σε μορφή θερμότητας προς το περιβάλλον.</li>\n  <li class='mb-2'>β. Ένα παράδειγμα εξώθερμης αντίδρασης είναι η καύση των ξύλων στο τζάκι.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "12. α. Ποιες αντιδράσεις χαρακτηρίζονται ενδόθερμες;<br>β. Να αναφέρετε ένα παράδειγμα ενδόθερμης αντίδρασης που σας είναι γνωστό από την καθημερινή σας ζωή.<br>γ. Σε ένα δοχείο στο οποίο υπάρχει θερμόμετρο πραγματοποιείται μία ενδόθερμη αντίδραση. Αν η αρχική θερμοκρασία στο δοχείο ήταν 50°C, η τελική θα είναι μεγαλύτερη, ίση ή μικρότερη από 50°C; Να δικαιολογήσετε την απάντησή σας.",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Ενδόθερμες ονομάζονται οι αντιδράσεις κατά τις οποίες απορροφάται ενέργεια σε μορφή θερμότητας από το περιβάλλον.</li>\n  <li class='mb-2'>β. Ένα παράδειγμα είναι η διάλυση αναβράζοντος δισκίου στο νερό ή το μαγείρεμα/ψήσιμο φαγητού.</li>\n  <li class='mb-2'>γ. Στις ενδόθερμες αντιδράσεις απορροφάται θερμότητα, άρα η θερμοκρασία του περιβάλλοντος μειώνεται. Οπότε η τελική θερμοκρασία του δοχείου θα είναι μικρότερη από 50°C.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "15. α. Το μεθάνιο καίγεται με το οξυγόνο του αέρα και παράγεται διοξείδιο του άνθρακα και νερό (CH<sub>4</sub> + 2O<sub>2</sub> -> CO<sub>2</sub> + 2H<sub>2</sub>O). Κατά την αντίδραση ορισμένης ποσότητας μεθανίου με 64g οξυγόνου παράχθηκαν 44g διοξείδιο του άνθρακα και 36g νερό. Ποια ήταν η ποσότητα μεθανίου που κάηκε και τι συμβαίνει στη θερμοκρασία του περιβάλλοντος;<br>β. Το άζωτο αντιδρά με υδρογόνο και παράγει αμμωνία. Αν αντιδράσουν 56g αζώτου με 12g υδρογόνου, πόση αμμωνία θα παραχθεί και τι θα συμβεί στη θερμοκρασία του περιβάλλοντος;",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Βάσει της αρχής διατήρησης της μάζας, η ποσότητα μεθανίου ήταν: 44g + 36g - 64g = 16g. Η καύση είναι εξώθερμη αντίδραση, άρα η θερμοκρασία του περιβάλλοντος αυξάνεται.</li>\n  <li class='mb-2'>β. Βάσει της αρχής διατήρησης της μάζας, θα παραχθούν 56g + 12g = 68g αμμωνίας. Κατά την αντίδραση εκλύεται θερμότητα (εξώθερμη) άρα η θερμοκρασία του περιβάλλοντος αυξάνεται.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "1. α. Με τι ασχολείται η χημική σύνθεση;<br>β. Να εξηγήσετε τους λόγους για τους οποίους οι επιστήμονες παρασκευάζουν συνθετικά ουσίες οι οποίες υπάρχουν στη φύση.",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Η χημική σύνθεση ασχολείται με τη μελέτη και την ανάλυση των συστατικών και της δομής χημικών ενώσεων, καθώς και τον σχεδιασμό/παραγωγή νέων ουσιών.</li>\n  <li class='mb-2'>β. Παρασκευάζουν συνθετικά ουσίες για: i) παραγωγή μεγάλων ποσοτήτων, ii) μελέτη των ιδιοτήτων μιας ουσίας, iii) αυξημένη καθαρότητα της ουσίας χωρίς άλλες προσμίξεις.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "2. Να περιγράψετε τα διακριτά στάδια της σύνθεσης του Buna (ενός είδους συνθετικού καουτσούκ) από ανόργανες πρώτες ύλες, με βάση τις παρακάτω μετατροπές:<br>Ανθρακασβέστιο (CaC<sub>2</sub>) -> Ακετυλένιο -> Βινυλακετυλένιο -> 1,3-βουταδιένιο -> Buna.",
    "a": "<span class='text-purple-300 font-bold'>Το ανθρακασβέστιο αντιδρά με νερό και παράγεται ακετυλένιο. Στη συνέχεια δύο μόρια ακετυλενίου αντιδρούν (διμερισμός) και παράγεται βινυλακετυλένιο. Το βινυλακετυλένιο αντιδρά με υδρογόνο και παράγεται 1,3-βουταδιένιο. Τελικά το 1,3-βουταδιένιο υφίσταται πολυμερισμό και παράγεται το πολυμερές Buna.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "3. α. Με τι ασχολείται η χημική ανάλυση;<br>β. Να εξηγήσετε τους λόγους για τους οποίους η χημική ανάλυση έμμεσα προστατεύει τους καταναλωτές από ενδεχόμενες νοθείες στα τρόφιμα, στα φάρμακα...",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Η χημική ανάλυση προσδιορίζει τη σύσταση των υλικών σωμάτων τόσο ποιοτικά (από ποια συστατικά αποτελούνται) όσο και ποσοτικά (σε ποιες ποσότητες/αναλογίες).</li>\n  <li class='mb-2'>β. Μέσω της χημικής ανάλυσης επιτυγχάνεται ο αυστηρός έλεγχος ποιότητας και ασφάλειας των καταναλωτικών προϊόντων (π.χ. φάρμακα, τρόφιμα), η υπολειμματική ανάλυση για επικίνδυνες προσμίξεις και η αποτροπή νοθείας.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "4. Πολλά βαρέα μέταλλα, όπως ο μόλυβδος (Pb), είναι τοξικά. Από την 1η Δεκεμβρίου 2013, το Υπουργείο Υγείας μείωσε το επιτρεπόμενο όριο μολύβδου στο πόσιμο νερό στα 10 μg/L.<br>α. Πώς ελέγχεται η ύπαρξη και η ποσότητα των βαρέων μετάλλων στο νερό;<br>β. Σε ένα χημικό εργαστήριο αναλύονται 10 mL του νερού μίας πηγής και ανιχνεύονται 2 &middot; 10<sup>-8</sup> g μολύβδου. Είναι κατάλληλο για πόσιμο αυτό το νερό;",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Η ύπαρξη και η ποσότητα ελέγχεται με τις μεθόδους της χημικής ανάλυσης.</li>\n  <li class='mb-2'>β. Στα 10 mL περιέχονται 2 &middot; 10<sup>-8</sup> g μολύβδου.<br>Άρα, στα 1000 mL (1 L) θα περιέχονται 100 &middot; 2 &middot; 10<sup>-8</sup> = 2 &middot; 10<sup>-6</sup> g μολύβδου, δηλαδή 2 μg/L.<br>Το επιτρεπόμενο όριο είναι 10 μg/L. Επομένως, το νερό αυτό είναι κατάλληλο για πόσιμο.</li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "5. α. Τι είναι η ρύπανση του περιβάλλοντος;<br>β. Ποιες ανθρώπινες δραστηριότητες επιβαρύνουν το περιβάλλον με ρύπους;<br>γ. Να συμπληρώσετε τους κυριότερους ρύπους, την πηγή προέλευσης και τις επιπτώσεις στο περιβάλλον.<br>(π.χ. CO<sub>2</sub>, SO<sub>2</sub>, λιπάσματα, πλαστικά, βαρέα μέταλλα).",
    "a": "<span class='text-purple-300 font-bold'><ul class='list-disc ml-5 mt-2'>\n  <li class='mb-2'>α. Η επιβάρυνση του εδάφους, των υδάτων και της ατμόσφαιρας με χημικές ουσίες (ρύπους/λύματα) από ανθρώπινες δραστηριότητες.</li>\n  <li class='mb-2'>β. Οι μεταφορές, η βιομηχανία, τα θερμοηλεκτρικά εργοστάσια, η αγροτική παραγωγή, τα αστικά απορρίμματα και η αστική χρήση του νερού.</li>\n  <li class='mb-2'>γ.\n  <div class=\"overflow-x-auto my-4\"><table class='w-full text-left border-collapse bg-white/5 rounded-xl overflow-hidden'>\n    <thead class='bg-white/10'>\n      <tr class='border-b border-white/20'><th class='py-3 px-4 font-semibold'>Ρύπος</th><th class='py-3 px-4 font-semibold'>Πηγή</th><th class='py-3 px-4 font-semibold'>Επιπτώσεις</th></tr>\n    </thead>\n    <tbody class=\"divide-y divide-white/10\">\n      <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>CO<sub>2</sub></td><td class='py-3 px-4'>Ορυκτά καύσιμα</td><td class='py-3 px-4'>Υπερθέρμανση γης</td></tr>\n      <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>SO<sub>2</sub> / NO<sub>x</sub></td><td class='py-3 px-4'>Βιομηχανία / Αυτοκίνητα</td><td class='py-3 px-4'>Όξινη βροχή, αναπνευστικά προβλήματα</td></tr>\n      <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Βαρέα μέταλλα</td><td class='py-3 px-4'>Βιομηχανία, βυρσοδεψεία</td><td class='py-3 px-4'>Εισαγωγή στην τροφική αλυσίδα</td></tr>\n      <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Λιπάσματα / Απορρυπαντικά</td><td class='py-3 px-4'>Αγροτική παραγωγή / Αστικά</td><td class='py-3 px-4'>Ευτροφισμός</td></tr>\n      <tr class='hover:bg-white/5 transition-colors'><td class='py-3 px-4'>Πλαστικά</td><td class='py-3 px-4'>Αστικά απόβλητα</td><td class='py-3 px-4'>Υποβάθμιση ωκεανών, εξαφάνιση ειδών</td></tr>\n    </tbody>\n  </table></div>\n  </li>\n</ul></span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "6. Να δικαιολογήσετε την ακόλουθη έκφραση: \"Η ιδιότητα του νερού να είναι εξαιρετικά καλός διαλύτης για πάρα πολλές ουσίες έχει πολύ σοβαρά μειονεκτήματα\".",
    "a": "<span class='text-purple-300 font-bold'>Αυτή η ιδιότητα του νερού έχει και σημαντικά μειονεκτήματα για την περιβαλλοντική υγεία, καθώς βαρέα μέταλλα, χημικά απόβλητα, οργανικές τοξίνες και άλλες επιβλαβείς ουσίες διαλύονται εύκολα σε αυτό και μεταφέρονται σε τεράστιες αποστάσεις μέσω του υδρολογικού κύκλου, μολύνοντας ευρύτερες περιοχές.</span>"
  },

  {
    "chapterId": "chapter-6",
    "q": "6. Να συμβουλευτείτε τα χρώματα των προσομοιωμάτων στην ενότητα 5 και στη συνέχεια να γράψετε τον μοριακό τύπο της χημικής ένωσης που αντιστοιχεί στο καθένα από τα προσομοιώματα του ακόλουθου πίνακα.<div class='overflow-x-auto my-4'><table class='border-collapse text-center text-sm bg-white/5 rounded-xl overflow-hidden w-full'><thead class='bg-white/10'><tr><th class='py-3 px-3 font-semibold'>1ο</th><th class='py-3 px-3 font-semibold'>2ο</th><th class='py-3 px-3 font-semibold'>3ο</th><th class='py-3 px-3 font-semibold'>4ο</th><th class='py-3 px-3 font-semibold'>5ο</th></tr></thead><tbody><tr><td class='py-4 px-3'><span style='display:inline-block;width:18px;height:18px;border-radius:50%;background:white;border:1px solid #aaa;vertical-align:middle;margin:2px;'></span><span style='display:inline-block;width:10px;height:2px;background:#888;vertical-align:middle;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#5B2D8E;vertical-align:middle;margin:2px;'></span><br><small class='text-white/60'>υδροϊώδιο</small></td><td class='py-4 px-3'><span style='display:inline-block;width:20px;height:20px;border-radius:50%;background:#e53e3e;vertical-align:middle;margin:2px;'></span><span style='display:inline-block;width:6px;height:2px;background:#888;vertical-align:middle;'></span><span style='display:inline-block;width:22px;height:22px;border-radius:50%;background:#718096;vertical-align:middle;margin:2px;'></span><span style='display:inline-block;width:6px;height:2px;background:#888;vertical-align:middle;'></span><span style='display:inline-block;width:20px;height:20px;border-radius:50%;background:#e53e3e;vertical-align:middle;margin:2px;'></span><br><small class='text-white/60'>διοξείδιο άνθρακα</small></td><td class='py-4 px-3'><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;vertical-align:middle;margin:2px;'></span><span style='display:inline-block;width:6px;height:2px;background:#888;vertical-align:middle;'></span><span style='display:inline-block;width:22px;height:22px;border-radius:50%;background:#ECC94B;vertical-align:middle;margin:2px;'></span><span style='display:inline-block;width:6px;height:2px;background:#888;vertical-align:middle;'></span><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;vertical-align:middle;margin:2px;'></span><br><small class='text-white/60'>υδρόθειο</small></td><td class='py-4 px-3'><span style='display:inline-block;width:20px;height:20px;border-radius:50%;background:#38a169;vertical-align:middle;margin:1px;'></span><span style='display:inline-block;width:20px;height:20px;border-radius:50%;background:#38a169;vertical-align:middle;margin:1px;'></span><span style='display:inline-block;width:22px;height:22px;border-radius:50%;background:#718096;vertical-align:middle;margin:1px;'></span><span style='display:inline-block;width:20px;height:20px;border-radius:50%;background:#38a169;vertical-align:middle;margin:1px;'></span><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;vertical-align:middle;margin:1px;'></span><br><small class='text-white/60'>χλωροφόρμιο</small></td><td class='py-4 px-3'><span style='display:inline-block;width:20px;height:20px;border-radius:50%;background:#e53e3e;vertical-align:middle;margin:2px;'></span><span style='display:inline-block;width:6px;height:2px;background:#888;vertical-align:middle;'></span><span style='display:inline-block;width:22px;height:22px;border-radius:50%;background:#3182ce;vertical-align:middle;margin:2px;'></span><span style='display:inline-block;width:6px;height:2px;background:#888;vertical-align:middle;'></span><span style='display:inline-block;width:20px;height:20px;border-radius:50%;background:#e53e3e;vertical-align:middle;margin:2px;'></span><br><small class='text-white/60'>διοξείδιο αζώτου</small></td></tr><tr class='text-xs text-white/50'><td class='pb-2'>λευκό=H, μωβ=I</td><td class='pb-2'>κόκκ.=O, γκρι=C</td><td class='pb-2'>λευκό=H, κίτρ.=S</td><td class='pb-2'>γκρι=C, πράσ.=Cl, λευκό=H</td><td class='pb-2'>κόκκ.=O, μπλε=N</td></tr></tbody></table></div>",
    "a": "<p class='font-bold text-emerald-400'>ΑΠΑΝΤΗΣΗ:</p><p class='text-emerald-400'>1ο (Υδροϊώδιο): HI<br>2ο (Διοξείδιο του άνθρακα): CO<sub>2</sub><br>3ο (Υδρόθειο): H<sub>2</sub>S<br>4ο (Χλωροφόρμιο): CHCl<sub>3</sub><br>5ο (Διοξείδιο του αζώτου): NO<sub>2</sub></p>"
  },

  {
    "chapterId": "chapter-4",
    "q": "10. Να συμπληρώσετε την ακροστιχίδα (Λύση). Η λέξη στην ακροστιχίδα σημαίνει χημική ουσία που δεν μπορεί να διασπαστεί σε απλούστερες.\n1. Τέτοια είναι η ποσοτική σύσταση των χημικών ενώσεων.\n2. Σημείο που είναι σταθερό για τις χημικές ενώσεις και τα χημικά στοιχεία.\n3. Τέτοια μείγματα είναι τα διαλύματα.\n4. Αυτές των ενώσεων είναι διαφορετικές από των συστατικών στοιχείων.\n5. Στοιχείο που είναι ακριβό μέταλλο.\n6. Χημική ουσία που μπορεί να διασπαστεί σε απλούστερες.\n7. Το σημείο ζέσης του χημικού στοιχείου ιώδιο είναι πάντα...\n8. Παράγεται κατά τη διάσπαση του νερού.",
    "a": "<div class='overflow-x-auto my-4'><table class='border-collapse text-sm'><thead><tr><th class='py-2 px-3 text-left text-white/60 font-normal'>#</th><th class='py-2 px-3 text-left text-white/60 font-normal'>Απάντηση</th></tr></thead><tbody><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>1.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Σ</span><span class='text-emerald-300'>ΤΑΘΕΡΗ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>2.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Τ</span><span class='text-emerald-300'>ΗΞΗΣ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>3.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ο</span><span class='text-emerald-300'>ΜΟΓΕΝΗ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>4.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ι</span><span class='text-emerald-300'>ΔΙΟΤΗΤΕΣ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>5.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Χ</span><span class='text-emerald-300'>ΡΥΣΟΣ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>6.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ε</span><span class='text-emerald-300'>ΝΩΣΗ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>7.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ι</span><span class='text-emerald-300'>ΔΙΟ</span></td></tr><tr class='border-t border-white/10'><td class='py-2 px-3 text-white/60'>8.</td><td class='py-2 px-3'><span class='inline-flex items-center justify-center w-7 h-7 rounded bg-yellow-400 text-gray-900 font-bold mr-1'>Ο</span><span class='text-emerald-300'>ΞΥΓΟΝΟ</span></td></tr></tbody></table></div><p class='mt-3 text-yellow-300 font-bold text-base'>Η λέξη της ακροστιχίδας είναι: <span class='tracking-widest text-yellow-400 text-lg'>ΣΤΟΙΧΕΙΟ</span></p>"
  },

  {
    "chapterId": "chapter-8",
    "q": "4. Στη διπλανή εικόνα δίνονται τα προσομοιώματα ορισμένων στοιχείων (H: λευκό, C: γκρι, N: μπλε, O: κόκκινο, Cl: πράσινο, Br: μωβ, I: σκούρο μωβ). Με βάση αυτά να αναπαραστήσετε τις ακόλουθες χημικές εξισώσεις με σύμβολα και λέξεις, αν δίνονται με προσομοιώματα, με λέξεις και προσομοιώματα, αν δίνονται με σύμβολα, και με σύμβολα και προσομοιώματα, αν δίνονται με λέξεις.<br><br><div class='flex flex-col gap-4'><div><b>α. Με προσομοιώματα:</b> <span class='inline-flex items-center gap-1'><span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:2;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#a855f7;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);margin-left:-6px;position:relative;z-index:1;'></span></span> <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:2;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#a855f7;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);margin-left:-6px;position:relative;z-index:1;'></span></span> <span class='mx-2'>&#10142;</span> <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#a855f7;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:2;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#a855f7;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);margin-left:-6px;position:relative;z-index:1;'></span></span> <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:2;'></span><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);margin-left:-4px;position:relative;z-index:1;'></span></span></span><br><b>Με χημική εξίσωση:</b> .....................................................</div><div><b>β. Με προσομοιώματα:</b> <span class='inline-flex items-center gap-1'><span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#3b82f6;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:2;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#3b82f6;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);margin-left:-6px;position:relative;z-index:1;'></span></span> <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:2;'></span><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);margin-left:-4px;position:relative;z-index:1;'></span></span> <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:2;'></span><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);margin-left:-4px;position:relative;z-index:1;'></span></span> <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:2;'></span><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);margin-left:-4px;position:relative;z-index:1;'></span></span> <span class='mx-2'>&#10142;</span> <span style='display:inline-flex;align-items:center;position:relative;'><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:1;margin-right:-4px;margin-top:8px;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#3b82f6;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:3;'></span><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:1;margin-left:-4px;margin-top:8px;'></span><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:absolute;top:-4px;left:15px;z-index:2;'></span></span> <span style='display:inline-flex;align-items:center;position:relative;'><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:1;margin-right:-4px;margin-top:8px;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#3b82f6;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:3;'></span><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:1;margin-left:-4px;margin-top:8px;'></span><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:absolute;top:-4px;left:15px;z-index:2;'></span></span></span><br><b>Με λέξεις:</b> .....................................................<br><b>Με χημική εξίσωση:</b> .....................................................</div><div><b>γ. Με λέξεις:</b> Ένα άτομο άνθρακα αντιδρά με δύο μόρια υδρογόνου και παράγεται ένα μόριο μεθανίου (CH<sub>4</sub>).<br><b>Με προσομοιώματα:</b> .....................................................<br><b>Με χημική εξίσωση:</b> .....................................................</div><div><b>δ. Με χημική εξίσωση:</b> N<sub>2</sub>(g) + 2O<sub>2</sub>(g) &#10142; 2NO<sub>2</sub>(g)<br><b>Με προσομοιώματα:</b> .....................................................<br><b>Με λέξεις:</b> .....................................................</div></div>",
    "a": "<p class='font-bold text-emerald-400'>ΑΠΑΝΤΗΣΗ:</p><p class='text-emerald-400'><b>α.</b> Με χημική εξίσωση: 2HBr &#10142; H<sub>2</sub> + Br<sub>2</sub><br><b>β.</b> Με χημική εξίσωση: N<sub>2</sub> + 3H<sub>2</sub> &#10142; 2NH<sub>3</sub><br><b>γ.</b> Με προσομοιώματα: <span class='inline-flex items-center gap-1'><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#4b5563;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);'></span> + <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:2;'></span><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);margin-left:-4px;position:relative;z-index:1;'></span></span> <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:2;'></span><span style='display:inline-block;width:16px;height:16px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);margin-left:-4px;position:relative;z-index:1;'></span></span> &#10142; <span style='display:inline-flex;align-items:center;position:relative;'><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:1;margin-right:-4px;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#4b5563;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:3;'></span><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:relative;z-index:1;margin-left:-4px;'></span><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:absolute;top:-4px;left:15px;z-index:2;'></span><span style='display:inline-block;width:14px;height:14px;border-radius:50%;background:white;border:1px solid #aaa;box-shadow:inset -2px -2px 4px rgba(0,0,0,0.2);position:absolute;bottom:-4px;left:15px;z-index:2;'></span></span></span><br>Με χημική εξίσωση: C + 2H<sub>2</sub> &#10142; CH<sub>4</sub><br><b>δ.</b> Με προσομοιώματα: <span class='inline-flex items-center gap-1'><span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#3b82f6;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:2;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#3b82f6;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);margin-left:-6px;position:relative;z-index:1;'></span></span> + <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#ef4444;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:2;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#ef4444;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);margin-left:-6px;position:relative;z-index:1;'></span></span> <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#ef4444;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:2;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#ef4444;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);margin-left:-6px;position:relative;z-index:1;'></span></span> &#10142; <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#ef4444;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:1;margin-right:-6px;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#3b82f6;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:2;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#ef4444;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);margin-left:-6px;position:relative;z-index:1;'></span></span> <span style='display:inline-flex;align-items:center;'><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#ef4444;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:1;margin-right:-6px;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#3b82f6;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);position:relative;z-index:2;'></span><span style='display:inline-block;width:24px;height:24px;border-radius:50%;background:#ef4444;box-shadow:inset -4px -4px 6px rgba(0,0,0,0.3);margin-left:-6px;position:relative;z-index:1;'></span></span></span><br>Με λέξεις: 1 μόριο αζώτου αντιδρά με δύο μόρια οξυγόνου και παράγουν 2 μόρια διοξειδίου του αζώτου.</p>"
  },
  {
    "chapterId": "chapter-8",
    "q": "8. Στη γλώσσα, όπως και στη χημεία χρησιμοποιούμε σύμβολα, ώστε να διευκολύνουμε την επικοινωνία. Να αντιστοιχίσετε τα σύμβολα της γλώσσας με τα σύμβολα της χημείας στα οποία πιστεύετε ότι αντιστοιχούν.\nΣτήλη Α: 1. γράμμα, 2. λέξη, 3. πρόταση\nΣτήλη Β: Α. χημική εξίσωση, Β. σύμβολο ατόμου, Γ. μοριακός τύπος",
    "a": "<p class='font-bold text-emerald-400'>ΑΠΑΝΤΗΣΗ:</p><p class='text-emerald-400'>1 -> Β (το γράμμα αντιστοιχεί στο σύμβολο του ατόμου)<br>2 -> Γ (η λέξη αντιστοιχεί στον μοριακό τύπο)<br>3 -> Α (η πρόταση αντιστοιχεί στη χημική εξίσωση)</p>"
  },];

export const bGymnasiouMCQ: MCQ[] = [
  {
    "chapterId": "chapter-1",
    "q": "3. α. Η λειτουργία του ανθρώπινου οργανισμού στηρίζεται σε πολύπλοκες χημικές μεταβολές που πραγματοποιούνται στο σώμα.",
    "options": [
      {
        "text": "Σωστό",
        "correct": true
      },
      {
        "text": "Λάθος",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΣΩΣΤΗ</span>"
  },
  {
    "chapterId": "chapter-1",
    "q": "3. β. Ο κόσμος που μας περιβάλλει αποτελείται από χημικές ουσίες.",
    "options": [
      {
        "text": "Σωστό",
        "correct": true
      },
      {
        "text": "Λάθος",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΣΩΣΤΗ</span>"
  },
  {
    "chapterId": "chapter-1",
    "q": "3. γ. Η Χημεία βοηθά τον άνθρωπο να αντιμετωπίσει τον πόνο.",
    "options": [
      {
        "text": "Σωστό",
        "correct": true
      },
      {
        "text": "Λάθος",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΣΩΣΤΗ</span>"
  },
  {
    "chapterId": "chapter-1",
    "q": "3. δ. Η Χημεία δεν έχει σχέση με τα τρόφιμα και τα υλικά που χρησιμοποιούνται στις κατασκευές.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΝΘΑΣΜΕΝΗ</span>"
  },
  {
    "chapterId": "chapter-1",
    "q": "3. ε. Ένα από τα αντικείμενα της Χημείας είναι η παραγωγή καλλυντικών και αρωμάτων.",
    "options": [
      {
        "text": "Σωστό",
        "correct": true
      },
      {
        "text": "Λάθος",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΣΩΣΤΗ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "2. α. Δεν υπάρχει πρόβλημα επάρκειας νερού, διότι το 71% της Γης καλύπτεται από νερό",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "2. β. Το νερό είναι απαραίτητο για την επιβίωση των ζώντων οργανισμών.",
    "options": [
      {
        "text": "Σωστό",
        "correct": true
      },
      {
        "text": "Λάθος",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΣΩΣΤΟ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "2. γ. Ορισμένα φυτά, όπως οι κάκτοι, μπορούν να επιβιώσουν χωρίς καθόλου νερό.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "2. δ. Η βιομηχανική ανάπτυξη του δυτικού κόσμου έχει αυξήσει κατά πολύ την κατανάλωση νερού.",
    "options": [
      {
        "text": "Σωστό",
        "correct": true
      },
      {
        "text": "Λάθος",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΣΩΣΤΟ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "2. ε. Τα στερεά τρόφιμα, όπως τα φρούτα, δεν περιέχουν νερό.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "5. α. Το νερό είναι το κύριο συστατικό:",
    "options": [
      {
        "text": "i. των φυτών",
        "correct": false
      },
      {
        "text": "ii. του ανθρώπινου σώματος",
        "correct": false
      },
      {
        "text": "iii. του γάλακτος",
        "correct": false
      },
      {
        "text": "iv. όλων των προηγούμενων",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση iv</span>"
  },
  {
      "chapterId": "chapter-3",
      "q": "2. α. Το νερό είναι εξαιρετικός διαλύτης και για τον λόγο αυτόν δεν βρίσκεται ποτέ στη φύση εντελώς καθαρό.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό</span>"
    },
    {
      "chapterId": "chapter-3",
      "q": "2. β. Το νερό παίζει σημαντικό ρόλο στη ρύθμιση της θερμοκρασίας του πλανήτη.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό</span>"
    },
    {
      "chapterId": "chapter-3",
      "q": "2. γ. Το νερό είναι το πιο άφθονο υγρό στη Γη.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό</span>"
    },
    {
      "chapterId": "chapter-3",
      "q": "2. δ. Το θαλασσινό νερό είναι κατάλληλο για την άρδευση (πότισμα) των καλλιεργειών.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Η υψηλή περιεκτικότητά του σε άλατα το καθιστά ακατάλληλο).</span>"
    },
    {
    "chapterId": "chapter-3",
    "q": "5. β. Το μεγαλύτερο ποσοστό του νερού στη Γη είναι:",
    "options": [
      {
        "text": "i. αλμυρό και βρίσκεται στους παγετώνες",
        "correct": false
      },
      {
        "text": "ii. γλυκό και βρίσκεται στους παγετώνες",
        "correct": false
      },
      {
        "text": "iii. αλμυρό και βρίσκεται στους ωκεανούς",
        "correct": true
      },
      {
        "text": "iv. πόσιμο και βρίσκεται στις λίμνες και στα ποτάμια",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση iii</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "10. α. Το ψωμί είναι ένα ομογενές μείγμα.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "10. β. Το θαλασσινό νερό είναι ένα μείγμα.",
    "options": [
      {
        "text": "Σωστό",
        "correct": true
      },
      {
        "text": "Λάθος",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΣΩΣΤΟ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "10. γ. Το αλατοπίπερο δεν είναι ούτε αλμυρό όπως το αλάτι, ούτε καυτερό όπως το πιπέρι.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "10. δ. Ο γρανίτης είναι ένα πέτρωμα τα συστατικά του οποίου είναι διακριτά ακόμη και με γυμνό μάτι, επομένως είναι ένα ομογενές μείγμα.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "10. ε. Δεν υπάρχουν στερεά μείγματα.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "10. στ. Για να εξαφανιστεί η γλυκιά γεύση της ζάχαρης, αρκεί να αναμειχθεί με νερό.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "10. ζ. Τα μείγματα σχηματίζονται με ανάμειξη των συστατικών τους σε καθορισμένη αναλογία.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "11. α. Ένα μείγμα μπορεί να βρίσκεται σε:",
    "options": [
      {
        "text": "i. σε στερεή κατάσταση",
        "correct": false
      },
      {
        "text": "ii. σε υγρή κατάσταση",
        "correct": false
      },
      {
        "text": "iii. σε αέρια κατάσταση",
        "correct": false
      },
      {
        "text": "iv. σε στερεή, υγρή ή αέρια κατάσταση",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση: iv</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "11. β. Τα ετερογενή μείγματα:",
    "options": [
      {
        "text": "i. έχουν συστατικά που δεν διακρίνονται με γυμνό μάτι ή μικροσκόπιο",
        "correct": false
      },
      {
        "text": "ii. έχουν ίδιες ιδιότητες σε όλη τους την έκταση",
        "correct": false
      },
      {
        "text": "iii. σχηματίζονται με ανάμειξη των συστατικών τους σε καθορισμένες αναλογίες",
        "correct": false
      },
      {
        "text": "iv. έχουν συστατικά που διακρίνονται οπωσδήποτε με μικροσκόπιο",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση: iv</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "11. γ. Με εξέταση του αίματος στο μικροσκόπιο διακρίνονται τα ερυθρά αιμοσφαίρια και το πλάσμα. Επομένως το αίμα είναι:",
    "options": [
      {
        "text": "i. ομογενές μείγμα",
        "correct": false
      },
      {
        "text": "ii. ετερογενές μείγμα",
        "correct": true
      },
      {
        "text": "iii. δεν είναι μείγμα",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση: ii</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "11. δ. Αν αναμείξουμε λάδι και νερό, θα σχηματιστούν δύο διακριτές φάσεις με το λάδι να επιπλέει πάνω στο νερό. Το σύστημα λάδι νερό είναι:",
    "options": [
      {
        "text": "i. διάλυμα",
        "correct": false
      },
      {
        "text": "ii. ομογενές μείγμα",
        "correct": false
      },
      {
        "text": "iii. ετερογενές μείγμα",
        "correct": true
      },
      {
        "text": "iv. κράμα",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση: iii</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "11. ε. Οι αφροί είναι μείγματα αεριών ουσιών σε υγρό διαλύτη των οποίων τα συστατικά διακρίνονται μόνο στο μικροσκόπιο. Επομένως οι αφροί είναι:",
    "options": [
      {
        "text": "i. διαλύματα",
        "correct": false
      },
      {
        "text": "ii. ομογενή μείγματα",
        "correct": false
      },
      {
        "text": "iii. ετερογενή μείγματα",
        "correct": true
      },
      {
        "text": "iv. κανένα από τα προηγούμενα",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση: iii</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "11. στ. Σε δύο ποτήρια τα οποία περιέχουν την ίδια ποσότητα νερού διαλύονται στο Δ<sub>1</sub> δύο κουταλιές αλάτι και στο Δ<sub>2</sub> μία κουταλιά αλάτι. Πιο πυκνό διάλυμα είναι:",
    "options": [
      {
        "text": "i. το Δ1",
        "correct": true
      },
      {
        "text": "ii. το Δ2",
        "correct": false
      },
      {
        "text": "iii. είναι το ίδιο πυκνά",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση: i</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "16. α. Για την παραλαβή αλατιού στις αλυκές χρησιμοποιείται η τεχνική της απόσταξης.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "16. β. Η εκχύλιση είναι η μέθοδος με την οποία οι αρωματικές ουσίες του γαλλικού καφέ περνούν στο ρόφημα από τους κόκκους του.",
    "options": [
      {
        "text": "Σωστό",
        "correct": true
      },
      {
        "text": "Λάθος",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΣΩΣΤΟ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "16. γ. Ένα ετερογενές μείγμα στερεών μπορεί να διαχωριστεί στα συστατικά του με διαλογή.",
    "options": [
      {
        "text": "Σωστό",
        "correct": true
      },
      {
        "text": "Λάθος",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΣΩΣΤΟ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "16. δ. Δεν είναι δυνατό να διαχωρίσουμε τις χρωστικές ενός μελανιού.",
    "options": [
      {
        "text": "Σωστό",
        "correct": false
      },
      {
        "text": "Λάθος",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>ΛΑΘΟΣ</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "22. α. Σε 1000 g διαλύματος ζάχαρης περιεκτικότητας 10% μάζα προς μάζα περιέχονται:",
    "options": [
      {
        "text": "i. 90 g ζάχαρης",
        "correct": false
      },
      {
        "text": "ii. 900 g ζάχαρης",
        "correct": false
      },
      {
        "text": "iii. 90 g νερό",
        "correct": false
      },
      {
        "text": "iv. 900 g νερό",
        "correct": true
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση: iv</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "22. β. Ένα υδατικό διάλυμα υδροχλωρικού οξέος 15% μάζα προς όγκο περιέχει:",
    "options": [
      {
        "text": "i. 15 g υδροχλωρίου σε 100 g διαλύματος",
        "correct": false
      },
      {
        "text": "ii. 15 g υδροχλωρίου σε 100 mL διαλύματος",
        "correct": true
      },
      {
        "text": "iii. 15 g υδροχλωρίου σε 100 mL νερό",
        "correct": false
      },
      {
        "text": "iv. 15 g νερό σε 100 mL διαλύματος",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση: ii</span>"
  },
  {
    "chapterId": "chapter-3",
    "q": "22. γ. Η θάλασσα είναι ένα διάλυμα που περιέχει εκτός από μαγειρικό αλάτι (χλωριούχο νάτριο) και άλλα άλατα με μικρότερη περιεκτικότητα. Η περιεκτικότητα του θαλασσινού νερού σε χλωρίδιο του μαγνησίου είναι περίπου 0,1% μάζα προς όγκο. 1 g χλωρίδιο του μαγνησίου θα περιέχεται σε:",
    "options": [
      {
        "text": "i. 1000 g θαλασσινού νερού",
        "correct": false
      },
      {
        "text": "ii. 1000 mL θαλασσινού νερού",
        "correct": true
      },
      {
        "text": "iii. 100 mL θαλασσινού νερού",
        "correct": false
      }
    ],
    "explanation": "<span class='text-rose-400 font-bold'>Σωστή απάντηση: ii</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "5. Α. Στο 1ο και 2ο μέλος μίας χημικής εξίσωσης είναι:",
    "options": [
      {
        "text": "Α. ίσο το άθροισμα του αριθμού μορίων των αντιδρώντων και των προϊόντων.",
        "correct": false
      },
      {
        "text": "Β. ίσο οι αριθμοί των ατόμων κάθε στοιχείου που μετέχει στην αντίδραση",
        "correct": false
      },
      {
        "text": "Γ. ίσο το άθροισμα του αριθμού των ατόμων των αντιδρώντων με το άθροισμα του αριθμού των ατόμων των προϊόντων.",
        "correct": true
      },
      {
        "text": "Δ. ίσοι οι αριθμοί των μορίων των αντιδρώντων και προϊόντων.",
        "correct": false
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Γ</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Β. Στη χημική εξίσωση: (NH<sub>4</sub>)2CO<sub>3</sub>(s) -> ...NH<sub>3</sub>(g) + ...CO<sub>2</sub>(g) + H<sub>2</sub>O(g), οι συντελεστές NH<sub>3</sub>, CO<sub>2</sub>, H<sub>2</sub>O είναι αντίστοιχα:",
    "options": [
      {
        "text": "Α. 1-1-1",
        "correct": false
      },
      {
        "text": "Β. 2-1-1",
        "correct": true
      },
      {
        "text": "Γ. 2-2-1",
        "correct": false
      },
      {
        "text": "Δ. κανένα από τα προηγούμενα",
        "correct": false
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Β</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Γ. Από τη χημική αντίδραση του υδρογόνου με το άζωτο παράγεται:",
    "options": [
      {
        "text": "Α. NH<sub>3</sub>",
        "correct": true
      },
      {
        "text": "Β. HBr",
        "correct": false
      },
      {
        "text": "Γ. H<sub>2</sub>O",
        "correct": false
      },
      {
        "text": "Δ. HCl",
        "correct": false
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Α</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Δ. Όταν αντιδρούν 64 g θείου (S) με 96 g οξυγόνου (O<sub>2</sub>), παράγονται:",
    "options": [
      {
        "text": "Α. 160 g CO<sub>2</sub>",
        "correct": false
      },
      {
        "text": "Β. 64 g SO<sub>2</sub>",
        "correct": false
      },
      {
        "text": "Γ. 160 g SO<sub>3</sub>",
        "correct": true
      },
      {
        "text": "Δ. 96 g SO<sub>2</sub>",
        "correct": false
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Γ</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "E. 245 g KClO<sub>3</sub>, όταν θερμαίνονται ισχυρά, διασπώνται και παράγουν 96 g O<sub>2</sub> και a KCI. Η αντίδραση είναι:",
    "options": [
      {
        "text": "Α. εξώθερμη",
        "correct": false
      },
      {
        "text": "Β. δεν συνοδεύεται από μεταβολές στην ενέργεια",
        "correct": false
      },
      {
        "text": "Γ. ενδόθερμη ή εξώθερμη",
        "correct": true
      },
      {
        "text": "Δ. ενδόθερμη",
        "correct": false
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Γ</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "E. (Μέρος 2) Από την αντίδραση (της προηγούμενης ερώτησης) παράχθηκαν:",
    "options": [
      {
        "text": "E. 56 g KCI",
        "correct": false
      },
      {
        "text": "Στ. 100 g KCI",
        "correct": false
      },
      {
        "text": "Z. 149 g KCI",
        "correct": true
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Ζ</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "Στ. Στην αντίδραση που περιγράφεται από την χημική εξίσωση: 2HI + Cl<sub>2</sub> -> ....... + ....... ο σωστός τρόπος αναδιάταξης των ατόμων είναι:",
    "options": [
      {
        "text": "Α. H<sub>2</sub> + ICl",
        "correct": false
      },
      {
        "text": "Β. HCl + HI",
        "correct": false
      },
      {
        "text": "Γ. 2HCl + I<sub>2</sub>",
        "correct": true
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Γ. 2HI + Cl<sub>2</sub> → 2HCl + I<sub>2</sub></span>"
  },
  {
    "chapterId": "chapter-7",
    "q": "5. Α. Οι ιοντικές ενώσεις αποτελούνται από:",
    "options": [
      {
        "text": "Α. μόρια",
        "correct": false
      },
      {
        "text": "Β. άτομα",
        "correct": false
      },
      {
        "text": "Γ. ιόντα",
        "correct": true
      },
      {
        "text": "Δ. συσσωματώματα μορίων",
        "correct": false
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Γ. Οι ιοντικές ενώσεις δεν αποτελούνται από μόρια, αλλά από θετικά και αρνητικά ιόντα τα οποία σχηματίζουν κρυστάλλους.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "5. Β. Το στοιχείο 15Ρ για να αποκτήσει σταθερή δομή μετατρέπεται σε:",
    "options": [
      {
        "text": "Α. κατιόν με φορτίο +3",
        "correct": false
      },
      {
        "text": "Β. ανιόν με φορτίο -3",
        "correct": true
      },
      {
        "text": "Γ. κατιόν με φορτίο -3",
        "correct": false
      },
      {
        "text": "Δ. ανιόν με φορτίο -2",
        "correct": false
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Β. Το στοιχείο 15Ρ έχει ηλεκτρονιακή δομή: K(2)-L(8)-M(5). Για να αποκτήσει σταθερή δομή προσλαμβάνει τρία ηλεκτρόνια και συμπληρώνει έτσι την εξωτερική του στιβάδα με 8 ηλεκτρόνια και μετατρέπεται σε ανιόν με φορτίο -3.</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "2.1. Από τις ακόλουθες προτάσεις δεν είναι σωστή για τον Π.Π. του Mendeleev.",
    "options": [
      {
        "text": "Α. Τα στοιχεία ταξινομήθηκαν από αυτό με τα ελαφρύτερα προς αυτό με τα βαρύτερα άτομα.",
        "correct": false
      },
      {
        "text": "B. O Mendeleev προέβλεψε την ύπαρξη στοιχείων που δεν είχαν ανακαλυφθεί.",
        "correct": false
      },
      {
        "text": "Γ. Τα στοιχεία ταξινομήθηκαν κατά αύξοντα ατομικό αριθμό.",
        "correct": true
      },
      {
        "text": "Δ. Στον Π.Π. του υπήρχαν κενές θέσεις.",
        "correct": false
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Γ</span>"
  },
  {
    "chapterId": "chapter-4",
    "q": "2.2. Στον Π.Π. του Mendeleev:",
    "options": [
      {
        "text": "Α. Οι οριζόντιες γραμμές ονομάστηκαν ομάδες.",
        "correct": false
      },
      {
        "text": "Β. Οι κατακόρυφες στήλες ονομάστηκαν περίοδοι και σε αυτές τοποθετήθηκαν στοιχεία με παρόμοιες ιδιότητες.",
        "correct": false
      },
      {
        "text": "Γ. Οι οριζόντιες γραμμές ονομάστηκαν περίοδοι και σε αυτές τοποθετήθηκαν στοιχεία με παρόμοιες ιδιότητες.",
        "correct": false
      },
      {
        "text": "Δ. Οι κατακόρυφες στήλες ονομάστηκαν ομάδες και σε αυτές τοποθετήθηκαν στοιχεία με παρόμοιες ιδιότητες.",
        "correct": true
      }
    ],
    "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: Δ</span>"
    },

    {
      "chapterId": "chapter-4",
      "q": "4. α. Αν κατά τη διάσπαση ορισμένης ποσότητας υδροϊωδίου σε υδρογόνο και ιώδιο παράγονται 2 g υδρογόνου και 254 g ιωδίου και κατά τη διάσπαση άλλης ποσότητας παράγονται 127 g ιωδίου, θα παράγεται ταυτόχρονα 1 g υδρογόνου.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "4. β. Ο άνθρακας δεν μπορεί να διασπαστεί σε άλλες απλούστερες ουσίες, επομένως είναι μία χημική ένωση.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Αφού δεν διασπάται, είναι χημικό στοιχείο).</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "4. γ. Το αλατόνερο περιέχει πολλά διαφορετικά χημικά στοιχεία, επομένως είναι μία χημική ένωση.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Είναι μείγμα).</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "4. δ. Μπορούμε να ανιχνεύσουμε αν ένα αέριο είναι το υδρογόνο πλησιάζοντας ένα μισοσβησμένο σπίρτο, γιατί θα το δούμε να ανάβει ξανά.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Αυτό ισχύει για το οξυγόνο. Το υδρογόνο καίγεται με χαρακτηριστικό κρότο).</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "4. ε. Κατά την ηλεκτρόλυση ορισμένης ποσότητας νερού παράγονται 0,5 L αερίου υδρογόνου και 0,5 L αερίου οξυγόνου.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Ο όγκος του υδρογόνου είναι διπλάσιος).</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "4. στ. Όταν οι χημικές ουσίες κατά τη διάσπασή τους παράγουν αέρια χημικά στοιχεία, η αναλογία των όγκων των αερίων στοιχείων είναι πάντα 2/1.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Εξαρτάται από τη χημική ένωση).</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "4. ζ. Το οινόπνευμα έχει σημείο βρασμού 78,5 °C, οπότε αν ένα σώμα αρχίζει να βράζει στους 78,5 °C, είναι οπωσδήποτε οινόπνευμα.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Μπορεί να είναι κάποιο μείγμα που τυχαία βράζει σε αυτή τη θερμοκρασία).</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "4. η. Οι χημικές ενώσεις έχουν καθορισμένες φυσικές σταθερές, ενώ τα χημικά στοιχεία όχι.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Και τα χημικά στοιχεία ως καθαρές ουσίες έχουν καθορισμένες φυσικές σταθερές).</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "4. θ. Το θαλασσινό νερό δεν έχει σταθερό σημείο βρασμού.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό (Ως μείγμα, βράζει σε εύρος θερμοκρασιών).</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "4. ι. Η αμμωνία αποτελείται από άζωτο και υδρογόνο με καθορισμένη αναλογία μαζών και γι' αυτό έχει τις ιδιότητες και του αζώτου και του υδρογόνου.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Ως χημική ένωση δεν διατηρεί τις ιδιότητες των συστατικών της).</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "5. α. Από τα ακόλουθα σώματα χημικό στοιχείο είναι το:",
      "options": [
        { "text": "i. γάλα", "correct": false },
        { "text": "ii. νερό", "correct": false },
        { "text": "iii. μαγειρικό αλάτι", "correct": false },
        { "text": "iv. Άνθρακας", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iv</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "5. β. Δεν είναι σωστή η πρόταση για μία χημική ένωση:",
      "options": [
        { "text": "i. Μπορεί να συντεθεί από απλούστερες ουσίες.", "correct": false },
        { "text": "ii. Έχει καθορισμένες φυσικές σταθερές σε ορισμένες συνθήκες.", "correct": false },
        { "text": "iii. Η αναλογία μαζών των στοιχείων που την αποτελούν είναι σταθερή.", "correct": false },
        { "text": "iv. Διατηρεί τις ιδιότητες των στοιχείων που την αποτελούν.", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iv</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "5. γ. Αν κατά τη διάσπαση της αμμωνίας η σχέση των όγκων αζώτου - υδρογόνου είναι 1/3, τότε όταν παράγονται 600 cm³ υδρογόνου ταυτόχρονα παράγονται από το άζωτο:",
      "options": [
        { "text": "i. 600 cm³", "correct": false },
        { "text": "ii. 1800 cm³", "correct": false },
        { "text": "iii. 200 cm³", "correct": true },
        { "text": "iv. 300 cm³", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iii</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "5. δ. Τα χημικά στοιχεία έχουν καθορισμένα και σταθερά:",
      "options": [
        { "text": "i. σημείο τήξης", "correct": false },
        { "text": "ii. σημείο βρασμού", "correct": false },
        { "text": "iii. σύσταση", "correct": false },
        { "text": "iv. τα α, β, γ", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iv</span>"
    },
    {
      "chapterId": "chapter-4",
      "q": "5. ε. Από τα ακόλουθα σώματα δεν έχει σταθερό σημείο τήξης:",
      "options": [
        { "text": "i. το χλωριούχο νάτριο", "correct": false },
        { "text": "ii. ο χρυσός", "correct": false },
        { "text": "iii. ο χάλυβας", "correct": true },
        { "text": "iv. ο μόλυβδος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iii (Ο χάλυβας είναι κράμα, δηλαδή μείγμα, άρα δεν έχει σταθερό σημείο τήξης).</span>"
    },

    {
      "chapterId": "chapter-5",
      "q": "3. α. Τα άτομα, σύμφωνα με τον Δημόκριτο, είναι:",
      "options": [
        { "text": "i. ακίνητα", "correct": false },
        { "text": "ii. χωρίς κενό", "correct": true },
        { "text": "iii. δημιουργούνται από το μηδέν", "correct": false },
        { "text": "iv. φθαρτά", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: ii (καθώς πίστευε ότι μέσα στα άτομα δεν υπάρχει κενό)</span>"
    },
    {
      "chapterId": "chapter-5",
      "q": "3. β. Τα άτομα, σύμφωνα με τον Dalton:",
      "options": [
        { "text": "i. είναι όλα ίδια", "correct": false },
        { "text": "ii. καταστρέφονται", "correct": false },
        { "text": "iii. δημιουργούνται από το μηδέν", "correct": false },
        { "text": "iv. έχουν ίδια μάζα αν ανήκουν στο ίδιο στοιχείο", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iv</span>"
    },
    {
      "chapterId": "chapter-6",
      "q": "5. α. Οι μοριακοί τύποι μας πληροφορούν για την αναλογία των ατόμων κάθε στοιχείου στο μόριο της ένωσης.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Μας πληροφορούν για τον ΑΚΡΙΒΗ αριθμό των ατόμων, όχι μόνο για την αναλογία).</span>"
    },
    {
      "chapterId": "chapter-6",
      "q": "5. β. Μοριακούς τύπους χρησιμοποιούμε μόνο για τις χημικές ενώσεις.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Και για τα στοιχεία που είναι διατομικά, πολυατομικά π.χ. O2, O3, P4).</span>"
    },
    {
      "chapterId": "chapter-6",
      "q": "5. γ. Η ατομικότητα ενός στοιχείου είναι ο αριθμός που μας δείχνει πόσα άτομα του στοιχείου μετέχουν στο μόριο της χημικής ένωσης.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Η ατομικότητα αναφέρεται στα μόρια των στοιχείων, όχι των ενώσεων. π.χ. στο Ο2 η ατομικότητα είναι 2).</span>"
    },
    {
      "chapterId": "chapter-6",
      "q": "8. α. Ο φωσφόρος είναι ένα στοιχείο το οποίο άλλοτε είναι τετρατομικό και άλλοτε εξατομικό. Το μόριό του λοιπόν συμβολίζεται με τους τύπους:",
      "options": [
        { "text": "i. P", "correct": false },
        { "text": "ii. P<sub>4</sub> ή P<sub>6</sub>", "correct": true },
        { "text": "iii. P<sub>6</sub>", "correct": false },
        { "text": "iv. P<sub>4</sub>", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: ii</span>"
    },
    {
      "chapterId": "chapter-6",
      "q": "8. β. Η γλυκόζη έχει μοριακό τύπο C<sub>6</sub>H<sub>12</sub>O<sub>6</sub>, επομένως:",
      "options": [
        { "text": "i. 6 άτομα άνθρακα συνδέονται με 12 άτομα υδρογόνου και 6 άτομα οξυγόνου, και σχηματίζονται δύο μόρια γλυκόζης.", "correct": false },
        { "text": "ii. 12 άτομα άνθρακα συνδέονται με 12 άτομα υδρογόνου και 6 άτομα οξυγόνου...", "correct": false },
        { "text": "iii. 12 άτομα άνθρακα συνδέονται με 24 άτομα υδρογόνου και 12 άτομα οξυγόνου...", "correct": false },
        { "text": "iv. 6 άτομα άνθρακα συνδέονται με 12 άτομα υδρογόνου και 6 άτομα οξυγόνου, και σχηματίζεται ένα μόριο γλυκόζης.", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iv</span>"
    },

    {
      "chapterId": "chapter-7",
      "q": "1. α. Οι απόψεις των επιστημόνων για τη δομή της ύλης δεν έχουν αλλάξει από τον 18ο αιώνα που ο Dalton διατύπωσε την ατομική του θεωρία.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Η θεωρία του Dalton βελτιώθηκε και αλλάχθηκε ριζικά με την ανακάλυψη των υποατομικών σωματιδίων).</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "1. β. Οι επιστήμονες ανέκαθεν πίστευαν ότι το άτομο αποτελείται κυρίως από κενό.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Ο Rutherford το απέδειξε πρώτος το 1911).</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "1. γ. Ο E. Rutherford ήταν ο πρώτος που μίλησε για τον πυρήνα και τα ηλεκτρόνια που περιφέρονται σε μεγάλη απόσταση από αυτόν.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "1. δ. Ο N. Bohr διατύπωσε πρώτος την άποψη ότι τα ηλεκτρόνια κινούνται με τυχαίο τρόπο γύρω από τον πυρήνα.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Υποστήριξε ότι κινούνται σε καθορισμένες κυκλικές τροχιές).</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "10. α. Για να μετατραπεί ένα άτομο σε κατιόν:",
      "options": [
        { "text": "i. προσλαμβάνει πρωτόνια", "correct": false },
        { "text": "ii. προσλαμβάνει ηλεκτρόνια", "correct": false },
        { "text": "iii. αποβάλλει πρωτόνια", "correct": false },
        { "text": "iv. αποβάλλει ηλεκτρόνια", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iv</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "10. β. Το ασβέστιο με ατομικό αριθμό 20 μετατρέπεται σε ιόν αποβάλλοντας δύο ηλεκτρόνια. Το φορτίο του ιόντος του ασβεστίου είναι:",
      "options": [
        { "text": "i. +1", "correct": false },
        { "text": "ii. +2", "correct": true },
        { "text": "iii. -2", "correct": false },
        { "text": "iv. κανένα από τα προηγούμενα", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: ii</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "10. γ. Το θείο έχει ατομικό αριθμό 16 και το ιόν του έχει 18 e. Το φορτίο του ιόντος είναι:",
      "options": [
        { "text": "i. +1", "correct": false },
        { "text": "ii. +2", "correct": false },
        { "text": "iii. -2", "correct": true },
        { "text": "iv. -1", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iii</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "13. α. Δομικά σωματίδια των χημικών ουσιών είναι μόνο τα μόρια.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Είναι και τα άτομα, και τα ιόντα).</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "13. β. Τα διατομικά στοιχεία αποτελούνται από ιόντα.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Αποτελούνται από μόρια, τα οποία αποτελούνται από άτομα).</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "13. γ. Τα ιόντα δημιουργούνται αν το άτομο προσλάβει ή αποβάλλει πρωτόνια ή ηλεκτρόνια.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (ΜΟΝΟ ηλεκτρόνια).</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "13. δ. Ένα άτομο με ατομικό αριθμό 17 έχει 17 νετρόνια στον πυρήνα του.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Έχει 17 πρωτόνια, όχι απαραίτητα νετρόνια).</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "13. ε. Ένα άτομο με μαζικό αριθμό 80 έχει μεγαλύτερη μάζα από ένα άτομο με μαζικό αριθμό 56.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "13. στ. Ένα άτομο μπορεί να έχει θετικό ή αρνητικό φορτίο ή να είναι ηλεκτρικά ουδέτερο.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Το άτομο είναι ΠΑΝΤΑ ηλεκτρικά ουδέτερο).</span>"
    },
    {
      "chapterId": "chapter-7",
      "q": "13. ζ. Αν γνωρίζουμε ότι το άτομο σιδήρου μπορεί να αποβάλλει 2 ή 3 ηλεκτρόνια, συμπεραίνουμε ότι σχηματίζει δύο διαφορετικά ιόντα.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό (Τα ιόντα Fe<sup>2+</sup> και Fe<sup>3+</sup>).</span>"
    }
,

    {
      "chapterId": "chapter-8",
      "q": "10. α. Σε μία χημική εξίσωση είναι ίσοι στο 1ο και 2ο μέλος:",
      "options": [
        { "text": "i. οι αριθμοί των μορίων των αντιδρώντων και προϊόντων", "correct": false },
        { "text": "ii. οι αριθμοί των ατόμων όλων των στοιχείων", "correct": true },
        { "text": "iii. το άθροισμα του αριθμού μορίων των αντιδρώντων και των προϊόντων", "correct": false },
        { "text": "iv. Ο αριθμός των μορίων των αντιδρώντων με τον αριθμό των ατόμων των προϊόντων", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: ii (Αρχή διατήρησης της μάζας - ατόμων)</span>"
    },
    {
      "chapterId": "chapter-8",
      "q": "10. β. Στη χημική εξίσωση: N<sub>2</sub>O<sub>4</sub>(g) -> ...NO<sub>2</sub>(g), ο συντελεστής του NO<sub>2</sub> είναι:",
      "options": [
        { "text": "i. 1", "correct": false },
        { "text": "ii. 4", "correct": false },
        { "text": "iii. 3", "correct": false },
        { "text": "iv. κανένα από τα προηγούμενα", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iv</span>"
    },
    {
      "chapterId": "chapter-8",
      "q": "10. γ. Από τη χημική αντίδραση του υδρογόνου με το χλώριο παράγεται:",
      "options": [
        { "text": "i. NH<sub>3</sub>", "correct": false },
        { "text": "ii. HBr", "correct": false },
        { "text": "iii. H<sub>2</sub>O", "correct": false },
        { "text": "iv. HCl", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iv</span>"
    },
    {
      "chapterId": "chapter-8",
      "q": "10. δ. Όταν καίγονται 16 g μεθανίου (CH<sub>4</sub>) με 64 g οξυγόνου (O<sub>2</sub>), παράγονται 36 g νερού (H<sub>2</sub>O) και ...",
      "options": [
        { "text": "i. 44 g NO<sub>2</sub>", "correct": false },
        { "text": "ii. 16 g CO<sub>2</sub>", "correct": false },
        { "text": "iii. 44 g CO<sub>2</sub>", "correct": true },
        { "text": "iv. 64 g CO<sub>2</sub>", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iii (Σύμφωνα με την αρχή διατήρησης της μάζας: 16+64 = 80g συνολικά αντιδρώντα. Προϊόντα = 36 + x = 80 => x = 44g CO2)</span>"
    },
    {
      "chapterId": "chapter-8",
      "q": "13. α. Οι φυσικές ιδιότητες των ουσιών που παράγονται από μία χημική αντίδραση είναι ίδιες με τις φυσικές ιδιότητες των αρχικών ουσιών.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Παράγονται νέες ουσίες με διαφορετικές ιδιότητες).</span>"
    },
    {
      "chapterId": "chapter-8",
      "q": "13. β. Κατά τη διάσπαση α g μίας ουσίας Α παράγονται α g μίας ουσίας Β και α g μίας ουσίας Γ.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος (Σε κάθε χημική αντίδραση η συνολική μάζα των αντιδρώντων είναι ίση με τη συνολική μάζα των προϊόντων. Εδώ θα ήταν x g της Β + (α-x) g της Γ).</span>"
    },
    {
      "chapterId": "chapter-8",
      "q": "13. γ. Όταν πραγματοποιείται μία χημική αντίδραση η θερμοκρασία του περιβάλλοντος μπορεί να αυξηθεί ή να ελαττωθεί.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό (Η θερμοκρασία αυξάνεται όταν η αντίδραση είναι εξώθερμη και ελαττώνεται όταν είναι ενδόθερμη).</span>"
    },
    {
      "chapterId": "chapter-8",
      "q": "14. β. 200 g ασβεστόλιθου (CaCO<sub>3</sub>) διασπώνται όταν θερμαίνονται ισχυρά και παράγουν 88 g διοξείδιο του άνθρακα και α g οξείδιο του ασβεστίου (CaO). Από την αντίδραση παράχθηκαν:",
      "options": [
        { "text": "i. 56 g CaO", "correct": false },
        { "text": "ii. 100 g CaO", "correct": false },
        { "text": "iii. 112 g CaO", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστή απάντηση: iii (200 = 88 + a => a = 112g)</span>"
    }
,

    {
      "chapterId": "chapter-9",
      "q": "7. α. Μπορούμε να χρησιμοποιούμε όση ποσότητα απορρυπαντικού θέλουμε χωρίς να επιβαρύνουμε τα υδάτινα οικοσυστήματα.",
      "options": [
        { "text": "Σωστό", "correct": false },
        { "text": "Λάθος", "correct": true }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Λάθος. Η υπερβολική χρήση απορρυπαντικών προκαλεί ευτροφισμό στα υδάτινα οικοσυστήματα.</span>"
    },
    {
      "chapterId": "chapter-9",
      "q": "7. β. Ο βιολογικός καθαρισμός μιμείται τη φυσική διαδικασία αυτοκαθαρισμού των υδάτινων οικοσυστημάτων.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό. Μιμείται και επιταχύνει τη διαδικασία αυτοκαθαρισμού χρησιμοποιώντας μικροοργανισμούς.</span>"
    },
    {
      "chapterId": "chapter-9",
      "q": "7. γ. Μπορούμε να πάρουμε μέτρα για την πρόληψη της ρύπανσης του νερού.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό. Με σωστή διαχείριση αποβλήτων και περιορισμό ρύπων.</span>"
    },
    {
      "chapterId": "chapter-9",
      "q": "7. δ. Η ρύπανση της θάλασσας με λύματα μπορεί να προκαλέσει σοβαρές βλάβες στους ανθρώπους, ακόμη και αν ζουν μακριά της.",
      "options": [
        { "text": "Σωστό", "correct": true },
        { "text": "Λάθος", "correct": false }
      ],
      "explanation": "<span class='text-emerald-400 font-bold'>Σωστό. Μέσω της τροφικής αλυσίδας οι ρύποι (π.χ. βαρέα μέταλλα) μπορούν να φτάσουν στον άνθρωπο.</span>"
    }
];