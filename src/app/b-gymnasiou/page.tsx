...
import { bGymnasiouChapters as chapters } from "@/data/curriculum";

export default function GradeBHub() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-800 to-blue-900 text-white font-sans flex flex-col relative overflow-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-[20%] -left-[10%] w-[50%] h-[50%] bg-cyan-400/20 blur-[120px] rounded-full"></div>
        <div className="absolute top-[40%] -right-[10%] w-[40%] h-[60%] bg-blue-400/20 blur-[120px] rounded-full"></div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-12 md:py-20 w-full">
        ...
        <header className="mb-16 md:mb-24 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            <motion.div 
              ...
              className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-cyan-100 text-sm font-medium mb-6 font-proxima"
            >
            ...
            >
              <a 
                ...
                className="group inline-flex items-center gap-4 md:gap-5 px-8 py-4 md:px-10 md:py-5 rounded-full bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-medium hover:from-cyan-400 hover:to-blue-400 transition-all shadow-xl shadow-cyan-500/25 hover:shadow-cyan-400/40 hover:-translate-y-1"
              >
          ...
          >
            <div className="bg-white/10 backdrop-blur-md border border-white/20 shadow-sm px-6 py-4 rounded-2xl flex flex-col items-center">
              ...
              <span className="text-xs text-cyan-200 uppercase tracking-wider mt-1 font-inter">Κεφάλαια</span>
            </div>
            <div className="bg-white/10 backdrop-blur-md border border-white/20 shadow-sm px-6 py-4 rounded-2xl flex flex-col items-center">
              <span className="text-3xl font-bold text-cyan-300 font-proxima">
                {chapters.reduce((sum, ch) => sum + ch.exercises, 0)}
              </span>
              <span className="text-xs text-cyan-200 uppercase tracking-wider mt-1 font-inter">Ερωτήσεις και Ασκήσεις</span>
            </div>
          </motion.div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {chapters.map((chapter, index) => (
            ...
            >
              <Link href={`/b-gymnasiou/${chapter.id}`} className="block h-full">
                <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl p-6 md:p-8 h-full transition-all duration-300 hover:-translate-y-1 hover:border-white/40 hover:shadow-xl hover:shadow-cyan-900/50 group relative overflow-hidden">
                  ...
                  <div className="flex flex-col md:flex-row gap-6 md:gap-8 h-full relative z-10">
                    ...
                    <div className="flex-grow flex flex-col">
                      <h3 className="text-2xl font-bold mb-3 text-white group-hover:text-cyan-200 transition-colors font-proxima">
                        {chapter.title}
                      </h3>
                      <p className="text-cyan-100 mb-6 text-sm leading-relaxed flex-grow font-inter">
                        {chapter.description}
                      </p>
                      
                      <div className="flex flex-wrap items-center justify-between gap-4 mt-auto">
                        <div className="flex gap-4">
                          <div className="flex items-center gap-1.5 text-xs text-cyan-200 font-inter font-medium">
                        ...
                        <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white group-hover:bg-cyan-500 group-hover:text-white transition-all shadow-sm">
